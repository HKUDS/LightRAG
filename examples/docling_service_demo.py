...
async def demo_fallback_processing():
    ...
    try:
        ...
        try:
            ...
            print(
                f"Processing time: {result['metadata'].get('processing_time_seconds', 0):.2f}s"
            )
            ...
            if result["success"]:
                ...
                print(
                    f"❌ Fallback processing failed: {result.get('error', 'Unknown error')}"
                )
        ...
        try:
            for test_file in test_files:
                ...
                (
                    use_service,
                    reason,
                ) = await docling_processor.should_use_docling_service(test_file)
                ...
                if result["success"]:
                    ...
                    print(
                        f"Service used: {metadata.get('docling_service_used', False)}"
                    )
                    print(
...
async def main():
    ...
    print(
        "   1. Start services: docker compose -f docker-compose.yml -f docker-compose.enhanced.yml --profile enhanced-processing up -d"
    )
    print(
        "   2. Wait for services to be ready (may take several minutes for first-time build)"
    )
