...
async def main():
    try:
        ...
        rag_params = {
            ...
            "addon_params": {"entity_types": ["language", "concept"]},
        }

        print(f"Using embedding model: {MODEL_NAME}")
        print(f"Using LLM model: {LLM_MODEL}")
        print(
            f"Using infinity engine: {INFINITY_ENGINE}, device: {INFINITY_DEVICE or 'default'}"
        )
        ...
        mix_param = QueryParam(
            mode="mix", top_k=1, only_need_context=True, json_response=True
        )
        ...
        print(
            f"Mix JSON Context: \n{json.dumps(mix_context, indent=2)}"
        )  # Pretty print JSON
    finally:
