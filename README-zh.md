...
| **addon_params** | `dict` | 附加参数，例如：`{"example_number": 1, "language": "Simplified Chinese", "entity_types": ["organization", "person", "geo", "event"]}`：设置示例限制、实体/关系提取的输出语言。可以通过参数 `--addon_params` 或 `.env` 文件中的 `ADDON_PARAMS` 提供，在两种情况下都必须提供有效的 `Json` 字符串，`jq -r` 工具可能会有所帮助。| `example_number: 所有示例, language: English` |
