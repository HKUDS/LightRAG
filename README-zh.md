...
| **addon_params** | `dict` | 附加参数，例如`{"example_number": 1, "language": "Simplified Chinese", "entity_types": ["organization", "person", "geo", "event"]}`：设置示例限制、输出语言和文档处理的批量大小;  `{"prompts_yaml_file": "path_to_your_domain_specific_prompts.yaml"}`: 导入用户自定义提示模板，例如根据应用领域提供专有实体类型及示例，请参看样板 `./lightrag/prompts.example.yaml`| `example_number: 所有示例, language: English` |
