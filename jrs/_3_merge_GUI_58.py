...
def fetch_entity_details(label):
    try:
        ...
        return {
            "desc": "Error: Server not reachable.", "type": "", "srcid": "", "fpath": "",
            "related_nodes": [], "edges": []
        }
    except Exception as e:
        print(f"Error fetching entity details for {label}: {e}")
        return {
            "desc": f"Error: {e}", "type": "", "srcid": "", "fpath": "",
            "related_nodes": [], "edges": []
        }
...
def update_relationship_api(source_id, target_id, new_description, new_keywords, weight=1.0):
    try:
...
def create_relationship_api(source_id, target_id, description, keywords, weight, source_file_id):
    try:
        ...
        asyncio.run(rag.ainsert_custom_kg(relationship_data, full_doc_id=source_file_id, file_path=source_file_id))
        print(f"Successfully created relationship from {source_id} to {target_id}.")
        return True
    except requests.exceptions.ConnectionError:
        messagebox.showerror("Connection Error", "Could not connect to LightRAG server to create relationship. Is it running?")
        return False
    except requests.exceptions.HTTPError as e:
        messagebox.showerror("API Error", f"Failed to create relationship from {source_id} to {target_id}: {e.response.status_code} - {e.response.text}")
        return False
    except Exception as e:
        messagebox.showerror("Error", f"An unexpected error occurred while creating relationship from {source_id} to {target_id}: {e}")
        return False
    finally:
        if 'rag' in locals():
...
class MergeGUI:
        ...
        try:
            if isinstance(widget, tk.Text):
                ...
                if widget.selection_present():  # Check if there's a selection
                    ...
                    selected_text = widget.get()  # Fallback to entire text if no selection
            else:
    ...
    def create_left_panel(self):
        ...
        ttk.Button(pagination_frame, text="First", command=lambda: self.change_page_to(0)).grid(row=0, column=0, padx=2)
        ttk.Button(pagination_frame, text="Previous", command=lambda: self.change_page(-1)).grid(row=0, column=1, padx=2)
        self.page_label = ttk.Label(pagination_frame, text="Page 1")
        self.page_label.grid(row=0, column=2, sticky="w")
        ttk.Button(pagination_frame, text="Next", command=lambda: self.change_page(1)).grid(row=0, column=3, padx=2)
        ttk.Button(pagination_frame, text="Last", command=self.go_to_last_page).grid(row=0, column=4, padx=2)
        
        ttk.Label(pagination_frame, text="Go to page:").grid(row=1, column=1, padx=(10, 2))
        self.page_entry = ttk.Entry(pagination_frame, width=5)
        self.page_entry.grid(row=1, column=2, padx=2)
        ttk.Button(pagination_frame, text="Go", command=self.jump_to_page).grid(row=1, column=3, padx=2)
        ...
        self.select_all_orphans_button = ttk.Button(action_buttons_frame, text="Select Orphans", command=self.select_all_orphans)
        self.select_all_orphans_button.grid(row=0, column=1, sticky="ew", padx=(2, 2))

        self.clear_selected_button = ttk.Button(action_buttons_frame, text="Clear Selected", command=self.clear_selected_entities)
        self.clear_selected_button.grid(row=0, column=2, sticky="ew", padx=(2, 2))

        self.show_selected_button = ttk.Button(action_buttons_frame, text="Show Selected", command=self.show_selected_only)
        self.show_selected_button.grid(row=1, column=0, sticky="ew", padx=(0, 2))

        self.clear_filter_button = ttk.Button(action_buttons_frame, text="Show All", command=self.clear_filter)
        self.clear_filter_button.grid(row=1, column=1, sticky="ew", padx=(2, 2))

        self.clear_all_button = ttk.Button(action_buttons_frame, text="Reset All", command=self.clear_all_selections)
    ...
    def create_entity_list(self):
        ...
        # Update page label
        total_pages = (len(self.filtered_entity_list) + self.page_size - 1) // self.page_size
    ...
    def create_right_panel(self):
        ...
        self.target_entry.bind("<Control-c>", lambda event: self.copy_selected_text(self.target_entry))
        ...
        self.entity_type.bind("<Control-c>", lambda event: self.copy_selected_text(self.entity_type))        
        ...
        self.create_rel_button = ttk.Button(button_row_frame, text="Create Relationship", command=self.open_create_relationship_modal)
    ...
    def update_selection(self):
        ...
        for label in selected:
            ...
            if self.entity_data[label].get("type") and not str(self.entity_data[label]["type"]).startswith("Error:"):
    ...
    def update_button_visibility(self):
        selected_from_all = {label for label, var in self.all_check_vars.items() if var.get()}
        
        if selected_from_all and set(self.filtered_entity_list) == selected_from_all and len(self.filtered_entity_list) == len(selected_from_all):
        ...
        if len(selected_from_all) == 2:
            ...
            has_relationship = any(
                (edge["source"] == entity1 and edge["target"] == entity2) or
                (edge["source"] == entity2 and edge["target"] == entity1)
                for edge in edges1
            )
    ...
    def calculate_tile_layout(self, available_width, available_height, num_items, min_height=600):
        if num_items == 0:
            return 0, 0, 0, 0
        ...
        if any_open:
            ...
            for idx, label in enumerate(selected):
                try:
                    ...
                    if related_nodes:
                        text_content_parts.append(f"Related Entities: {len(related_nodes)}")
    ...
    def open_create_relationship_modal(self):
        selected = [label for label, var in self.all_check_vars.items() if var.get()]
        if len(selected) != 2:
            messagebox.showerror("Error", "Please select exactly two entities to create a relationship.")
            return
        ...
        ttk.Label(modal, text="Target Entity:").grid(row=3, column=0, sticky="w", padx=2)
        ...
        save_button = ttk.Button(button_frame, text="Save",
                                command=lambda: self.save_new_relationship(
                                    source_var.get(), target_var.get(), desc_text, keywords_entry.get(),
                                    weight_var.get(), source_file_id_entry.get(), modal))
        save_button.pack(side="right", padx=(5,0))
    ...
    def save_new_relationship(self, source_id, target_id, desc_text, keywords, weight, source_file_id, modal):
        ...
        if not source_id or not target_id or not description or not keywords or not weight or not source_file_id:
        ...
        if create_relationship_api(source_id, target_id, description, keywords, weight_float, source_file_id):
    ...
    def open_edit_relationships_modal(self, entity_label):
        ...
        ttk.Label(edit_frame, text="Weight (1.0-10.0):").grid(row=4, column=0, sticky="w", padx=5, pady=2)
        ...
        save_button = ttk.Button(button_frame, text="Save",
                                 command=lambda: self.save_relationship(entity_label, filtered_edges, relationship_combobox.current(), desc_text, keywords_entry, weight_var.get(), modal))
        save_button.pack(side="right", padx=(5,0))
        ...
        delete_button = ttk.Button(
            ...
            command=lambda: asyncio.run(
                self.delete_relationship(entity_label, filtered_edges, relationship_combobox.current(), modal)
            )
        )
    ...
    def save_relationship(self, entity_label, edges, selected_idx, desc_text, keywords_entry, weight, modal):
        if selected_idx < 0:
        ...
        if update_relationship_api(source_id, target_id, new_description, new_keywords, weight_float):
    ...
    async def delete_relationship(self, entity_label, edges, selected_idx, modal):
        ...
        try:
            ...
            messagebox.showinfo("Success", f"Deleted relationship from '{source_id}' to '{target_id}'.")
    ...
    def submit_merge(self):
        ...
        if not (set(selected) == selected_from_filtered and len(selected) == len(selected_from_filtered)):
            messagebox.showerror("Operation Not Allowed", "Merge operation can only be performed when 'Selected Only' mode is active and the displayed list exactly matches the selected entities.")
            return
    ...
    async def run_delete(self):
        selected_for_deletion = [label for label, var in self.all_check_vars.items() if var.get()]
        ...
        if len(selected_for_deletion) > 1:
            confirm_message = f"Are you sure you want to delete the following {len(selected_for_deletion)} entities?\n\n" + "\n".join(selected_for_deletion[:10]) + ("..." if len(selected_for_deletion) > 10 else "")
        else:
            confirm_message = f"Are you sure you want to delete '{selected_for_deletion[0]}'?"
        ...
        try:
            ...
            if success_count > 0:
                messagebox.showinfo("Deletion Complete", f"Successfully deleted {success_count} entities. Failed to delete {fail_count} entities. Please check logs for details.")
                if failed_entities:
                    messagebox.showerror("Deletion Errors", "Failed to delete:\n" + "\n".join(failed_entities))
                ...
                messagebox.showerror("Deletion Failed", "No entities were successfully deleted. Please check logs for details.")
