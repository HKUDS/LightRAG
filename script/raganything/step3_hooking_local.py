...
logger.add(os.path.join(LOG_DIR, f"step3_ollama_{time.strftime('%Y%m%d_%H%M%S')}.log"), rotation="10 MB", encoding="utf-8")
...
async def main():
    ...
    logger.info(f"🚀 初始化 LightRAG (LLM: Azure | Embed: Ollama/bge-m3)...")
    ...
    for i, json_file_path in enumerate(all_json_files):
        ...
        for j, (page_num, page_content) in enumerate(sorted_pages):
            ...
            if len(buffer_content) >= TARGET_CHUNK_SIZE or is_last_page:
                ...
                try:
                    # 1. 正常插入內容 (讓 LightRAG 自動切分和提取實體)
                    await rag.ainsert(final_text, file_paths=source_id)
                    chunk_count += 1
                    
                    # === 🔥 [Method 1] Graph Hook 插入 (修正版) ===
                    if is_trusted:
                        custom_kg = {
                            "entities": [
                                {
                                    ...
                                    "source_id": source_id  # 指向下面的 Chunk
                                },
                                # 我們也把 Document 本身作為一個實體節點加入，方便連接
                                {
                                    "entity_name": source_id, 
                                    "entity_type": "DocumentChunk",
                                    "description": f"Content from {doc_label}, marked as verified.",
                                    "source_id": source_id
                                }
                            ],
                            "relationships": [
                            ...
                            # 🔥 關鍵修正：這裡必須提供 Chunk 的內容，讓 source_id 有地方對應
                            "chunks": [
                                {
                                    "content": final_text,
                                    "source_id": source_id,
