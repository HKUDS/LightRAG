...
TRUSTED_KEYWORDS = ["HSBC", "Annual_Report"] 
# =================
...
logger.add(os.path.join(LOG_DIR, f"step3_reranker_tag_{time.strftime('%Y%m%d_%H%M%S')}.log"), rotation="10 MB", encoding="utf-8")
...
if not SF_API_KEY:
    logger.error("❌ 找不到 API Key")
    sys.exit(1)

async def main():
    if not os.path.exists(STEP2_BASE_DIR):
        logger.error(f"❌ 找不到 Step 2 輸出目錄")
        return

    all_json_files = glob.glob(os.path.join(STEP2_BASE_DIR, "*", "granular_content.json"))
    if not all_json_files: return

    if not os.path.exists(WORKING_DIR): os.makedirs(WORKING_DIR)
    
    logger.info("🚀 初始化 LightRAG (Reranker Prep Version)...")
    
    # 注意：這裡不需要注入 rerank_model_func，因為我們只是在做 Insert
    # Reranker 函數是在 Query 時才需要的
    rag = LightRAG(
    ...
    for i, json_file_path in enumerate(all_json_files):
        ...
        if is_trusted:
            logger.info(f"   ⭐ 識別為可信來源 (將注入 <TRUSTED> 標籤)")
        ...
        for j, (page_num, page_content) in enumerate(sorted_pages):
            ...
            if len(buffer_content) >= TARGET_CHUNK_SIZE or is_last_page:
                ...
                # === 🔥 [Method 2] Reranker Tag Injection ===
                # 如果是可信文件，在內容最前面加上特殊標籤
                # 你的 Rerank Function 之後會檢查這個標籤並加分
                if is_trusted:
                    final_text = f"<TRUSTED>\nSource: {source_id}\n\n{buffer_content}"
                else:
