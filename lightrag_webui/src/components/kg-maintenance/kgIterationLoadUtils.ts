...
const optionalDisplayArtifact = async (
  ...
  try {
    const displayArtifact = await loaders.getDisplayArtifact(requestWorkspace, artifactKey)
    return {
      key: artifactKey,
...
export async function loadKGMaintenanceWorkspaceBundle(
  ...
  const [
    ...
    optionalDisplayArtifact(
      requestWorkspace,
      'llm_review_trace',
      resolvedLoaders,
      'application/json',
      () => resolvedLoaders.getTrace(requestWorkspace)
    ),
    optionalDisplayArtifact(
      requestWorkspace,
      'llm_review_report',
      resolvedLoaders,
      'text/markdown',
      () => resolvedLoaders.getReport(requestWorkspace)
    ),
    optionalDisplayArtifact(
      requestWorkspace,
      'proposals_generated',
      resolvedLoaders,
      'text/markdown',
      () => resolvedLoaders.getProposals(requestWorkspace)
    ),
    optionalDisplayArtifact(
      requestWorkspace,
      'llm_judge_report',
      resolvedLoaders,
      'text/markdown',
      () => resolvedLoaders.getJudgeReport(requestWorkspace)
    ),
