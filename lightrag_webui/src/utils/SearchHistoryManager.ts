...
const STORAGE_KEY = 'lightrag_search_history'
const MAX_HISTORY = searchHistoryMaxItems
const VERSION = searchHistoryVersion
 ...
 */
export function addToHistory(label: string): void {
 ...
 */
export function getRecentSearches(limit = 10): SearchHistoryItem[] {
  const history = getHistory()
 ...
 */
export function getPopularRecommendations(limit?: number): SearchHistoryItem[] {
  const history = getHistory()
 ...
 */
export function getHistoryLabels(limit?: number): string[] {
  const history = getHistory()
 ...
 */
export function removeLabel(label: string): void {
...
// Re-export as namespace object for backward compatibility
