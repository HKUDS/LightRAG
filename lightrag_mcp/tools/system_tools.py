...
from ..utils import Validator, MCPError, format_timestamp, parse_time_range
...
async def lightrag_clear_cache(
    ...
    try:
        ...
        lightrag_cache_types = [ct for ct in cache_types 
        ...
        if lightrag_cache_types:
            if config.enable_direct_mode:
                ...
                result = {"cleared_caches": [], "message": "Cache clearing not available in direct mode"}
            else:
                async with get_api_client(config) as client:
...
async def lightrag_get_system_stats(
    ...
    if stat_categories:
        valid_categories = ["queries", "documents", "resources", "performance", "errors"]
        invalid_categories = [cat for cat in stat_categories if cat not in valid_categories]
        if invalid_categories:
            raise MCPError("INVALID_PARAMETER", 
                          f"Invalid stat categories: {invalid_categories}")
    else:
    ...
    try:
        ...
        if stat_categories and len(stat_categories) < 5:  # Not all categories
            filtered_result = {
                ...
                if any(cat in key for cat in stat_categories) or 
                   key in ["time_range", "timestamp", "mcp_server_info"]
            }
