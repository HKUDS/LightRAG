...
async def lightrag_get_graph(
    ...
    logger.debug(f"Getting graph data: max_nodes={max_nodes}, max_edges={max_edges}, "
...
async def lightrag_get_entity_relationships(
    ...
    if direction not in valid_directions:
        raise MCPError("INVALID_PARAMETER", 
                      f"Invalid direction. Valid options: {valid_directions}")
    ...
    if relationship_types:
        ...
        if not all(isinstance(rt, str) for rt in relationship_types):
            raise MCPError("INVALID_PARAMETER", "All relationship types must be strings")
    ...
    try:
        ...
        relationship_count = len(result.get("relationships", []))
        logger.info(f"Retrieved {relationship_count} relationships for entity: {entity_id}")
        return result
