...
async def lightrag_insert_text(
    ...
    try:
        ...
        logger.info(
            f"Text document inserted successfully: {result.get('document_id', 'unknown')}"
        )
        return result
...
async def lightrag_insert_file(
    ...
    if processing_options:
        if not isinstance(processing_options, dict):
            raise MCPError(
                "INVALID_PARAMETER", "Processing options must be a dictionary"
            )
        ...
        if "chunk_size" in processing_options:
            ...
            if not isinstance(chunk_size, int) or chunk_size <= 0:
                raise MCPError(
                    "INVALID_PARAMETER", "chunk_size must be a positive integer"
                )
    ...
    try:
        ...
        if config.enable_direct_mode:
            async with LightRAGDirectClient(config) as client:
                result = await client.insert_file(
        ...
        logger.info(
            f"File inserted successfully: {result.get('document_id', 'unknown')}"
        )
        return result
...
async def lightrag_delete_documents(
    document_ids: List[str], cascade_delete: bool = True, create_backup: bool = False
) -> Dict[str, Any]:
    """
    ...
    try:
        ...
        logger.info(
            f"Deletion completed: {deleted_count} successful, {failed_count} failed"
        )
        return result
...
async def lightrag_batch_process(
    items: List[Dict[str, Any]], max_concurrent: int = 5, stop_on_error: bool = False
) -> Dict[str, Any]:
    """
    ...
    for i, item in enumerate(items):
        ...
        if item["item_type"] not in ["file", "text", "url"]:
            raise MCPError(
                "INVALID_PARAMETER", f"Item {i} has invalid type: {item['item_type']}"
            )
        ...
        batch_result = {
            ...
            "statistics": {
                ...
                "total_processing_time": sum(
                    r.get("processing_time", 0) or 0 for r in processed_results
                ),
