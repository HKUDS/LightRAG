...
class LightRAGAPIClient:
    ...
    async def _handle_response(self, response: httpx.Response, operation: str) -> Dict[str, Any]:
        ...
        try:
            ...
            logger.debug(f"Operation {operation} successful", 
            ...
            logger.error(f"HTTP {response.status_code} error in {operation}: {error_detail}", 
                        extra={"correlation_id": correlation_id})
            
            # Map HTTP status codes to MCP errors
            if response.status_code == 401:
                raise MCPError("UNAUTHORIZED", "Authentication required or invalid", 
                             {"status_code": response.status_code, "detail": error_detail})
            elif response.status_code == 403:
                raise MCPError("FORBIDDEN", "Operation not permitted", 
                             {"status_code": response.status_code, "detail": error_detail})
            elif response.status_code == 404:
                raise MCPError("NOT_FOUND", "Resource not found", 
                             {"status_code": response.status_code, "detail": error_detail})
            elif response.status_code == 429:
                raise MCPError("RATE_LIMITED", "Rate limit exceeded", 
                             {"status_code": response.status_code, "detail": error_detail})
            elif response.status_code >= 500:
                raise MCPError("SERVICE_UNAVAILABLE", "LightRAG service unavailable", 
                             {"status_code": response.status_code, "detail": error_detail})
            else:
                raise MCPError("API_ERROR", f"API request failed: {e}", 
                             {"status_code": response.status_code, "detail": error_detail})
        
        except (ConnectError, TimeoutException) as e:
            logger.error(f"Connection error in {operation}: {e}", 
                        extra={"correlation_id": correlation_id})
            raise MCPError("LIGHTRAG_UNAVAILABLE", 
    ...
    async def stream_query(self, query: str, mode: str = "hybrid", **params) -> AsyncIterator[str]:
    ...
    # Document operations
    async def insert_text(self, text: str, title: str = "", metadata: Optional[Dict] = None) -> Dict[str, Any]:
    ...
    async def list_documents(self, status_filter: str = "", limit: int = 50, offset: int = 0) -> Dict[str, Any]:
        ...
        logger.debug(f"Listing documents: limit={limit}, offset={offset}, status={status_filter}")
    ...
    async def search_entities(self, query: str, limit: int = 20, **params) -> Dict[str, Any]:
        """Search entities."""
        client = await self._ensure_client()
    ...
    async def get_entity_relationships(self, entity_id: str, **params) -> Dict[str, Any]:
        """Get entity relationships."""
        client = await self._ensure_client()
        
        logger.debug(f"Getting relationships for entity: {entity_id}")
        
        response = await client.get(f"/graph/entities/{entity_id}/relationships", params=params)
