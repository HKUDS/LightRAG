...
try:
    ...
    print(f"Error: MCP library not installed. Please install with: pip install mcp", file=sys.stderr)
...
from .tools.document_tools import (
    lightrag_insert_text, lightrag_insert_file, lightrag_list_documents,
    lightrag_delete_documents, lightrag_batch_process
)
from .tools.graph_tools import (
    lightrag_get_graph, lightrag_search_entities, lightrag_update_entity,
    lightrag_get_entity_relationships
)
from .tools.system_tools import (
    lightrag_health_check, lightrag_clear_cache, lightrag_get_system_stats
)
...
class LightRAGMCPServer:
    ...
    def _setup_tools(self):
        ...
        if self.config.enable_document_upload:
            ...
            async def lightrag_insert_text_tool(
                ...
                return await lightrag_insert_text(text=text, title=title, metadata=metadata)
        ...
        @self.mcp.tool()
        async def lightrag_clear_cache_tool(
        ...
        async def get_document_status() -> str:
            ...
            try:
                ...
                status_summary = {
                    ...
                    "pipeline_status": {
                        "active": True,
                        "mode": "api" if not self.config.enable_direct_mode else "direct"
                    }
                }
    ...
    async def run(self):
        ...
        try:
            ...
            logger.info(f"Configuration: API URL={self.config.lightrag_api_url}, "
                       f"Mode={'direct' if self.config.enable_direct_mode else 'api'}")
            logger.info(f"Features: streaming={self.config.enable_streaming}, "
            ...
            try:
                ...
                logger.info(f"Startup health check: {health_result.get('status', 'unknown')}")
            except Exception as e:
