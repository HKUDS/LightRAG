...
class LightRAGMCPConfig:
    ...
    def from_env(cls) -> "LightRAGMCPConfig":
        ...
        config = cls(
            ...
            # Feature flags
            enable_direct_mode=get_env_bool(
                "MCP_ENABLE_DIRECT_MODE", cls.enable_direct_mode
            ),
            enable_streaming=get_env_bool("MCP_ENABLE_STREAMING", cls.enable_streaming),
            enable_graph_modification=get_env_bool(
                "MCP_ENABLE_GRAPH_MODIFICATION", cls.enable_graph_modification
            ),
            enable_document_upload=get_env_bool(
                "MCP_ENABLE_DOCUMENT_UPLOAD", cls.enable_document_upload
            ),
            # Security settings
            require_auth=get_env_bool("MCP_REQUIRE_AUTH", cls.require_auth),
            allowed_file_types=get_env_list(
                "MCP_ALLOWED_FILE_TYPES",
                [".txt", ".md", ".pdf", ".docx", ".pptx", ".xlsx", ".html", ".json"],
            ),
            max_file_size_mb=get_env_int("MCP_MAX_FILE_SIZE_MB", cls.max_file_size_mb),
            max_documents_per_batch=get_env_int(
                "MCP_MAX_DOCUMENTS_PER_BATCH", cls.max_documents_per_batch
            ),
            # Performance settings
            default_query_timeout=get_env_int(
                "MCP_DEFAULT_QUERY_TIMEOUT", cls.default_query_timeout
            ),
            max_concurrent_queries=get_env_int(
                "MCP_MAX_CONCURRENT_QUERIES", cls.max_concurrent_queries
            ),
            cache_enabled=get_env_bool("MCP_CACHE_ENABLED", cls.cache_enabled),
            cache_ttl_seconds=get_env_int(
                "MCP_CACHE_TTL_SECONDS", cls.cache_ttl_seconds
            ),
            # Query defaults
            default_query_mode=os.getenv(
                "MCP_DEFAULT_QUERY_MODE", cls.default_query_mode
            ),
            default_top_k=get_env_int("MCP_DEFAULT_TOP_K", cls.default_top_k),
            default_chunk_top_k=get_env_int(
                "MCP_DEFAULT_CHUNK_TOP_K", cls.default_chunk_top_k
            ),
            default_cosine_threshold=get_env_float(
                "MCP_DEFAULT_COSINE_THRESHOLD", cls.default_cosine_threshold
            ),
            default_max_tokens=get_env_int(
                "MCP_DEFAULT_MAX_TOKENS", cls.default_max_tokens
            ),
            # HTTP client settings
            http_timeout=get_env_int("MCP_HTTP_TIMEOUT", cls.http_timeout),
            http_max_connections=get_env_int(
                "MCP_HTTP_MAX_CONNECTIONS", cls.http_max_connections
            ),
            http_max_keepalive=get_env_int(
                "MCP_HTTP_MAX_KEEPALIVE", cls.http_max_keepalive
            ),
            # Logging settings
            log_level=os.getenv("MCP_LOG_LEVEL", cls.log_level).upper(),
            log_format=os.getenv("MCP_LOG_FORMAT", cls.log_format),
            enable_debug_logging=get_env_bool(
                "MCP_ENABLE_DEBUG_LOGGING", cls.enable_debug_logging
            ),
        )
    ...
    def __post_init__(self):
        ...
        logger.info("LightRAG MCP Server Configuration:")
        ...
        logger.info(
            f"  Features: streaming={self.enable_streaming}, "
            f"graph_mod={self.enable_graph_modification}, "
            f"doc_upload={self.enable_document_upload}"
        )
        logger.info(
            f"  Security: auth_required={self.require_auth}, "
