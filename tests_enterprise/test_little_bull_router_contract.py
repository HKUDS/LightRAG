...
def test_little_bull_popular_labels_limit_matches_frontend_contract():
    ...
    openapi = app.openapi()
    parameters = openapi["paths"]["/little-bull/graph/label/popular"]["get"][
        "parameters"
    ]
...
def test_little_bull_group_routes_exist_contract():
    ...
    assert {"get", "post"}.issubset(
        openapi["paths"]["/little-bull/knowledge-subgroups"]
    )
...
def test_little_bull_canvas_routes_exist_contract():
    ...
    assert (
        "post" in openapi["paths"]["/little-bull/canvas/boards/{canvas_board_id}/nodes"]
    )
    assert (
        "post" in openapi["paths"]["/little-bull/canvas/boards/{canvas_board_id}/edges"]
    )
    assert (
        "get"
        in openapi["paths"]["/little-bull/canvas/boards/{canvas_board_id}/analysis"]
    )
    assert (
        "post"
        in openapi["paths"]["/little-bull/canvas/boards/{canvas_board_id}/dossier"]
    )
    assert "get" in openapi["paths"]["/little-bull/dossiers"]
    assert "get" in openapi["paths"]["/little-bull/dossiers/{knowledge_dossier_id}"]
    assert (
        "post"
        in openapi["paths"]["/little-bull/dossiers/{knowledge_dossier_id}/export"]
    )
    ...
    assert {"format", "destination", "approval_id", "include_audit"}.issubset(
        dossier_export_props
    )
...
def test_little_bull_content_map_and_trail_routes_exist_contract():
    ...
    assert (
        "/little-bull/knowledge-trails/{knowledge_trail_id}/steps" in openapi["paths"]
    )
    ...
    assert (
        "get" in openapi["paths"]["/little-bull/knowledge-trails/{knowledge_trail_id}"]
    )
    assert (
        "post"
        in openapi["paths"]["/little-bull/knowledge-trails/{knowledge_trail_id}/steps"]
    )
...
def test_little_bull_inbox_and_daily_note_routes_exist_contract():
    ...
    assert {"get", "post"}.issubset(
        openapi["paths"]["/little-bull/curator/suggestions"]
    )
    assert (
        "post"
        in openapi["paths"]["/little-bull/curator/suggestions/{inbox_item_id}/apply"]
    )
    ...
    curator_response_props = schemas["LittleBullCuratorSuggestionResponse"][
        "properties"
    ]
    ...
    assert {"inbox_item", "requires_approval", "allowed_actions"}.issubset(
        curator_response_props
    )
...
def test_little_bull_legal_matter_extraction_routes_exist_contract():
    ...
    assert (
        "/little-bull/legal/extractions/{legal_matter_extraction_run_id}/review"
        in openapi["paths"]
    )
    assert (
        "/little-bull/legal/extractions/{legal_matter_extraction_run_id}"
        in openapi["paths"]
    )
...
def test_little_bull_agent_builder_and_context_budget_routes_exist_contract():
    ...
    cost_params = {
        parameter["name"]
        for parameter in openapi["paths"]["/little-bull/costs/summary"]["get"][
            "parameters"
        ]
    ...
    assert {
        ...
        "group_id",
        "subgroup_id",
    }.issubset(cost_params)
    assert {"get", "post"}.issubset(
        openapi["paths"]["/little-bull/admin/agent-builder/sessions"]
    )
    assert (
        "post"
        in openapi["paths"][
    ...
    assert {"get", "post"}.issubset(
        openapi["paths"]["/little-bull/admin/agents/context-budgets"]
    )
    ...
    operational_request_props = schemas["LittleBullOperationalChatRequest"][
        "properties"
    ]
    operational_response_props = schemas["LittleBullOperationalChatResponse"][
        "properties"
    ]
    graph_params = {
        parameter["name"]
        for parameter in openapi["paths"]["/little-bull/graph/obsidian"]["get"][
            "parameters"
        ]
        if parameter.get("in") == "query"
    }
    ...
    assert {"document_ids", "top_k", "reserved_response_tokens"}.issubset(
        estimate_request_props
    )
    assert {
    ...
    assert {
        ...
        "origin_type",
        "max_nodes",
    }.issubset(graph_params)
    assert {"nodes", "edges", "clusters", "trails", "chat_context"}.issubset(
        graph_response_props
    )
    assert {
        "agent_id",
        "group_id",
    ...
    assert {
        ...
        "note",
        "suggestion",
    }.issubset(operational_response_props)
