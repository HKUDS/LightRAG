...
def _install_runtime(monkeypatch, repo, principals):
    ...
    monkeypatch.setattr(
        runtime, "get_system_auth_service", lambda: FakeEnterpriseAuth(principals)
    )
...
@pytest.mark.asyncio
async def test_core_mutable_route_blocks_enterprise_token_without_permission(
    monkeypatch,
):
    import sys
...
def test_core_router_wiring_uses_activity_specific_dependencies():
    ...
    document_routes = (
        repo_root / "lightrag/api/routers/document_routes.py"
    ).read_text()
    ...
    assert '"/scan"' in document_routes
    assert "response_model=ScanResponse" in document_routes
    assert "dependencies=[Depends(document_upload_auth)]" in document_routes
