...
def _client(
    repo: InMemorySystemRepository, principal: Principal, monkeypatch
) -> TestClient:
...
def test_master_can_set_hosted_private_llm_exception_policy(monkeypatch):
    ...
    response = client.post(
        "/system/policies/private-data/hosted-llm-exception", json=_payload()
    )
    ...
    policy = repo.policies[
        ("default", "default", PRIVATE_DATA_HOSTED_LLM_EXCEPTION_POLICY)
    ]
    ...
    assert (
        events[-1].action
        == "little_bull.policies.private_data.hosted_llm_exception.update"
    )
...
def test_policy_endpoint_rejects_expired_policy(monkeypatch):
    ...
    response = client.post(
        "/system/policies/private-data/hosted-llm-exception", json=payload
    )
