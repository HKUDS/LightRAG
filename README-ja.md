...
- **EMBEDDING_CHUNK_OVERLAP_TOKEN_SIZE**: チャンク分割後もチャンクが埋め込みモデルのコンテキスト上限を超えている場合に、埋め込みのハードフォールバックが直前のウィンドウから借用するトークンのオーバーラップ量を制御します。チャンカー自体のオーバーラップ設定である `CHUNK_OVERLAP_SIZE` とは独立しています（一部のチャンク分割戦略は無関係な理由でその値をゼロにするため、このフォールバックは独自の値を保持します）。デフォルトは `100`、`0` でこのフォールバックのオーバーラップを無効化します。
