...
| **addon_params** | `dict` | Additional parameters, e.g., `{"example_number": 1, "language": "Simplified Chinese", "entity_types": ["organization", "person", "geo", "event"]}`: sets example limit, entity/relation extraction output language. can be supplied via argument `--addon_params` or `ADDON_PARAMS` in `.env` file, in both cases a valid `Json` string must be supplied, `jq -r` might be helpful.| `example_number: all examples, language: English` |
