...
| **use_structured_extraction** | `bool` | Use structured JSON output for entity/relationship extraction (avoids delimiter parsing errors). Native schema support: OpenAI, Azure OpenAI, Gemini; others use JSON mode or prompt-only. Set `false` for legacy format. | `true` |
...
| **addon_params** | `dict` | Additional parameters, e.g., `{"language": "Simplified Chinese", "entity_types": ["Person", "Organization", "Location"]}`: sets example limit, entity/relation extraction output language, and custom entity types. When `entity_types` is set with structured extraction, providers with native structured output (OpenAI, Azure OpenAI, Gemini) use schema-level enforcement so the LLM cannot return types outside this list. Others (Ollama, Bedrock, Anthropic, etc.) rely on prompt instructions. See [CLAUDE.md](CLAUDE.md#llm-structured-output-support-matrix) for the full provider matrix. Also configurable via `ENTITY_TYPES` env var (JSON array). | `language: English` |
