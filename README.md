...
- **ENABLE_ENTITY_AUTO_MERGE**: Opt-in (default off). After each document is indexed, checks its newly-extracted entities against existing same-type entities already in the graph, using a bounded top-N similarity search followed by an LLM disambiguation check, and auto-merges confirmed duplicates (e.g. `"Acme Corp"` and `"Acme Corporation"`). The LLM call records an explicit verdict for every candidate, so a near-duplicate that is genuinely a different entity (e.g. `"Apple Inc."` vs. `"Apple fruit"`) is logged as a detected conflict rather than silently left alone or wrongly merged. `ENTITY_MERGE_TOP_K` bounds the candidate count per entity (default 3); `ENTITY_MERGE_SIMILARITY_THRESHOLD` is a best-effort pre-filter applied only on backends that expose a comparable similarity figure (default 0.85). Not applied when inserting through `ainsert_custom_kg`.
