...
| **addon_params** | `dict` | Additional parameters, e.g., `{"example_number": 1, "language": "Simplified Chinese", "entity_types": ["organization", "person", "geo", "event"]}`: sets example limit, entiy/relation extraction output language; `{"prompts_yaml_file": "path_to_your_domain_specific_prompts.yaml"}`: load user-defined prompts template, e.g. domain specific entity types and extraction examples, see `./lightrag/prompts.example.yaml` for details | `example_number: all examples, language: English` |
