...
def run_tests():
    ...
    doc1_id = next(
        (d["id"] for d in docs if "Flamingo" in d.get("content_summary", "")), None
    )
    doc2_id = next(
        (d["id"] for d in docs if "Eagle" in d.get("content_summary", "")), None
    )
    ...
    resp1_after = client.query(
        tenant_id, kb_id, "What is the codename for Project Flamingo?"
    )
