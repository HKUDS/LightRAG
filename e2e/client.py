...
class LightRAGClient:
    ...
    def wait_for_indexing(self, tenant_id, kb_id, timeout=300):
        ...
        raise Exception(
            f"Timeout ({timeout}s) waiting for indexing. Last status: {last_status}"
        )
    ...
    def clear_cache(self, tenant_id, kb_id):
        ...
        headers = {"X-Tenant-ID": tenant_id, "X-KB-ID": kb_id}
        response = self.session.post(
            f"{self.base_url}/documents/clear_cache", json={}, headers=headers
        )
        if response.status_code != 200:
    ...
    def wait_for_pipeline(self, tenant_id, kb_id, timeout=60):
        print_step(
            f"Waiting for pipeline to be idle in Tenant: {tenant_id}, KB: {kb_id}..."
        )
        headers = {"X-Tenant-ID": tenant_id, "X-KB-ID": kb_id}
        start_time = time.time()
        while time.time() - start_time < timeout:
            response = self.session.get(
                f"{self.base_url}/documents/pipeline_status", headers=headers
            )
            if response.status_code != 200:
    ...
    def query(self, tenant_id, kb_id, query_text, verbose=True):
        ...
        if verbose:
            print(
                f"  Response ({elapsed:.1f}s): {response_text[:150]}{'...' if len(response_text) > 150 else ''}"
            )
