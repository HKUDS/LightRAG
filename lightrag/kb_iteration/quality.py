...
from .models import KGSnapshot, QualityFinding, QualityScore, SnapshotEdge
...
def evaluate_snapshot_quality(snapshot: KGSnapshot) -> QualityScore:
    ...
    missing_node_source_count = sum(1 for node in snapshot.nodes if not node.source_id)
    missing_node_file_path_count = sum(1 for node in snapshot.nodes if not node.file_path)
    missing_edge_source_count = sum(1 for edge in snapshot.edges if not edge.source_id)
    missing_edge_file_path_count = sum(1 for edge in snapshot.edges if not edge.file_path)
    ...
    metrics = {
        ...
        "missing_node_source_count": missing_node_source_count,
        "missing_node_file_path_count": missing_node_file_path_count,
        "missing_edge_source_count": missing_edge_source_count,
        "missing_edge_file_path_count": missing_edge_file_path_count,
...
def _is_generic_relation(edge: SnapshotEdge) -> bool:
    tokens = [token.strip().casefold() for token in edge.keywords.split(",")]
...
def _hierarchy_metrics(snapshot: KGSnapshot) -> dict[str, int]:
    required_labels = [category.label for category in TOP_LEVEL_MEDICAL_CATEGORIES]
    ...
    present = sum(
        1 for label in required_labels if label in node_labels or label in node_ids
    )
...
def _disease_hub_overload_ratio(snapshot: KGSnapshot) -> float:
    ...
    disease_ids = {
        node.id for node in snapshot.nodes if node.entity_type.casefold() == "disease"
    }
    if not disease_ids:
        return 0.0
...
def _build_findings(
    *,
    ...
    missing_evidence = [
        "node.source_id" if metrics["missing_node_source_count"] else "",
        "node.file_path" if metrics["missing_node_file_path_count"] else "",
        "edge.source_id" if metrics["missing_edge_source_count"] else "",
        "edge.file_path" if metrics["missing_edge_file_path_count"] else "",
    ]
