...
class MemgraphStorage(BaseGraphStorage):
            ...
            try:
                async with self._driver.session(database=DATABASE) as session:
                    ...
                    try:
                        ...
                        logger.info(f'[{self.workspace}] Created index on :{workspace_label}(entity_id) in Memgraph.')
                ...
                logger.error(f'[{self.workspace}] Failed to connect to Memgraph at {URI}: {e}')
                raise
    ...
    async def upsert_node(self, node_id: str, node_data: dict[str, str]) -> None:
        ...
        for attempt in range(max_retries):
            try:
                logger.debug(f'[{self.workspace}] Attempting node upsert, attempt {attempt + 1}/{max_retries}')
                ...
                if is_transient:
                    ...
                    logger.error(f'[{self.workspace}] Non-transient error during node upsert: {e!s}')
                    raise
            except Exception as e:
                logger.error(f'[{self.workspace}] Unexpected error during node upsert: {e!s}')
                raise
    ...
    async def upsert_edge(self, source_node_id: str, target_node_id: str, edge_data: dict[str, str]) -> None:
        ...
        for attempt in range(max_retries):
            try:
                logger.debug(f'[{self.workspace}] Attempting edge upsert, attempt {attempt + 1}/{max_retries}')
                async with self._driver.session(database=self._DATABASE) as session:
                ...
                if is_transient:
                    ...
                    # Non-transient error, don't retry
                    logger.error(f'[{self.workspace}] Non-transient error during edge upsert: {e!s}')
                    raise
            except Exception as e:
                logger.error(f'[{self.workspace}] Unexpected error during edge upsert: {e!s}')
                raise
    ...
    async def get_knowledge_graph(
        ...
        async with self._driver.session(database=self._DATABASE, default_access_mode='READ') as session:
            try:
                ...
                logger.warning(f'[{self.workspace}] Memgraph error during subgraph query: {e!s}')
