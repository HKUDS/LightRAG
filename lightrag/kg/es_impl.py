...
class ESClientManager:
    ...
    @staticmethod
    async def create_index_if_not_exist_kv_docstatus(client: AsyncElasticsearch, index_name: str):
        mapping = {
            "mappings": {
                "properties": {
                    "id": {"type": "keyword"},
    ...
    @staticmethod
    async def create_index_if_not_exist_vector(client: AsyncElasticsearch, index_name: str, dims: int = 1024):
        mapping = {
            "mappings": {
                "properties": {
                    "id": {"type": "keyword"},
...
class ESKVStorage(BaseKVStorage):
    ...
    async def initialize(self):
        if self._client is None:
            ...
            await ESClientManager.create_index_if_not_exist_kv_docstatus(self._client, self._index)
    ...
    async def get_by_mode_and_id(self, mode: str, id: str) -> Union[Dict[str, Any], None]:
        if is_namespace(self.namespace, NameSpace.KV_STORE_LLM_RESPONSE_CACHE):
            key = f"{mode}_{id}"
            try:
    ...
    async def drop(self) -> Dict[str, str]:
        try:
            await self._client.delete_by_query(index=self._index, body={"query": {"match_all": {}}})
...
class ESDocStatusStorage(DocStatusStorage):
    ...
    async def get_by_ids(self, ids: List[str]) -> List[Dict[str, Any]]:
        ...
        return [
            ...
            for doc in docs if doc.get("found")
        ]
    ...
    async def get_status_counts(self) -> Dict[str, int]:
        ...
        res = await self._client.search(index=self._index, body={"size": 0, "aggs": aggs})
        return {
    ...
    async def get_docs_by_status(self, status: DocStatus) -> Dict[str, DocProcessingStatus]:
        ...
        res = await self._client.search(index=self._index, body={"query": query, "size": 10000})
    ...
    async def delete(self, ids: list[str]) -> None:
        if not ids:
            return
        actions = [
            {"delete": {"_index": self._index, "_id": doc_id}}
            for doc_id in ids
        ]
        try:
            await async_bulk(self._client, actions, refresh="wait_for")
...
class ESVectorDBStorage(BaseVectorStorage):
    ...
    async def initialize(self):
        if self._client is None:
            ...
            await ESClientManager.create_index_if_not_exist_vector(
                self._client,
                self._index,
                dims=self.embedding_func.embedding_dim
            )
    ...
    async def delete_entity_relation(self, entity_name: str) -> None:
        ...
        try:
            ...
            await self._client.delete_by_query(index=self._index, body=query, refresh=True)
        ...
        try:
            ...
            for doc in docs:
                if doc.get("found"):
                    results.append({
                        "id": doc["_id"],
                        "created_at": doc["_source"].get("meta", {}).get("created_at"),
                        **doc["_source"]["meta"]
                    })
            return results
        except Exception as e:
