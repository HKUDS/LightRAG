...
class RedisTenantHelper:
    ...
    def parse_tenant_key(tenant_key: str) -> Dict[str, str]:
        ...
        if len(parts) == 3:
            return {"tenant_id": parts[0], "kb_id": parts[1], "original_key": parts[2]}
    ...
    def batch_make_tenant_keys(
        tenant_id: str, kb_id: str, keys: List[str]
    ) -> List[str]:
        """Create multiple tenant-scoped keys at once"""
        return [
            RedisTenantHelper.make_tenant_key(tenant_id, kb_id, key) for key in keys
        ]
    ...
    async def scan_keys(self, pattern: str = "*") -> List[str]:
        ...
        while True:
            cursor, batch = await self.redis.scan(
                cursor, match=tenant_pattern, count=100
            )
            keys.extend(
                [
                    RedisTenantHelper.extract_original_key(
        ...
        for key_bytes in keys:
            ...
            try:
                ...
                tenant_key = RedisTenantHelper.make_tenant_key(
                    default_tenant_id, default_kb_id, key
                )
