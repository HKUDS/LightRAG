...
def process_edge_properties(
    ...
    if "extraction_timestamp" not in processed or processed["extraction_timestamp"] is None:
        processed["extraction_timestamp"] = datetime.now().isoformat()
    
    # Add description if missing
    if "description" not in processed or not processed["description"]:
        processed["description"] = f"Relationship of type {rel_type} between {source_id} and {target_id}"
...
def construct_relationship_queries(
    rel_types: Union[str, List[str]] = None,
    ...
    if rel_types:
        ...
        if len(rel_types) == 1:
            ...
            neo4j_types = "|".join([standardize_relationship_type(rt) for rt in rel_types])
