    ...
    try:
        ...
        async with driver.session(database=database) as session:
            ...
            while offset < total_relationships:
                ...
                if not dry_run:
                    for rel in relationships:
                        ...
                        try:
                            ...
                            stats["errors"] += 1
                            logger.error(f"Error migrating relationship {rel_id}: {str(e)}")
                else:
                ...
                logger.info(f"Progress: {processed}/{total_relationships} ({progress:.1f}%)")
    ...
    try:
        async with driver.session(database=database) as session:
            ...
            if stats["total_relationships"] > 0:
                non_legacy = stats["total_relationships"] - stats["legacy_relationships"]
                stats["percent_migrated"] = (non_legacy / stats["total_relationships"]) * 100
            else:
            ...
            if verbose:
                ...
                logger.info(f"Legacy RELATED relationships: {stats['legacy_relationships']}")
            ...
            for rel_type, count in stats["relationship_types"].items():
                ...
                while offset < count:
                    ...
                    async for record in batch_result:
                        ...
                        relationship_type = original_type or rel_type_prop or rel_type.lower()
                    ...
                    if not dry_run:
                        for rel in relationships:
                            ...
                            try:
                                ...
                                stats["errors"] += 1
                                logger.error(f"Error rolling back relationship {rel_id}: {str(e)}")
                    else:
                        # In dry run mode, just log what would be done
                        if verbose:
                            for rel in relationships:
                    ...
                    logger.info(f"Progress for {rel_type}: {offset}/{count} ({progress:.1f}%)")
