...
from .relationship_registry import RelationshipTypeRegistry, standardize_relationship_type
...
class RelationshipExtractor:
    ...
    async def extract_relationships(self, text: str) -> List[Dict[str, Any]]:
        ...
        """
        logger.debug(f"Starting relationship extraction from text of length {len(text)} characters")
        
        # Get valid relationship types from the registry
        relationship_types = ", ".join(self.relationship_registry.get_all_relationship_types())
        logger.debug(f"Using {len(self.relationship_registry.get_all_relationship_types())} relationship types from registry")
        ...
        logger.debug(f"Formatted extraction prompt with {len(formatted_prompt)} characters")
        ...
        try:
            ...
            try:
                ...
                if not isinstance(relationships, list):
                    logger.warning("LLM returned non-list response for relationship extraction")
                    relationships = []
                else:
                    logger.info(f"Successfully parsed {len(relationships)} relationships from LLM response")
            except json.JSONDecodeError:
                logger.warning("Failed to parse LLM response as direct JSON, trying pattern extraction")
                ...
                if json_match:
                    try:
                        ...
                        if not isinstance(relationships, list):
                            ...
                            logger.info(f"Successfully extracted {len(relationships)} relationships using pattern matching")
                    except json.JSONDecodeError:
                        logger.warning("Failed to parse JSON from LLM response pattern match")
                        relationships = []
                else:
            ...
            if len(relationships) > 0:
                ...
                logger.info(f"Relationship extraction completed: {processed_count}/{len(relationships)} relationships processed successfully ({success_rate:.1f}% success rate)")
            else:
                logger.info("Relationship extraction completed: No relationships found in text")
            ...
            if processed_relationships:
                ...
                type_summary = ", ".join([f"{rt}: {count}" for rt, count in rel_types.items()])
            ...
            logger.error(f"Text length: {len(text)}, Available relationship types: {len(self.relationship_registry.get_all_relationship_types())}")
            return []
    ...
    def _process_relationships(self, relationships: List[Dict[str, Any]]) -> List[Dict[str, Any]]:
        ...
        """
        logger.debug(f"Processing {len(relationships)} raw relationships for validation and standardization")
        ...
        for i, rel in enumerate(relationships):
            ...
            if not all(k in rel for k in ["source", "target", "relationship_type"]):
                logger.warning(f"Skipping relationship {i+1} with missing required fields: {rel}")
            ...
            logger.debug(f"Standardized relationship type '{rel_type}' to Neo4j type '{neo4j_type}'")
            ...
            if isinstance(weight, str):
                try:
                    weight = float(weight)
                    logger.debug(f"Converted string weight '{original_weight}' to float {weight}")
                except ValueError:
            ...
            if weight < 0 or weight > 1:
                if weight <= 10:  # Likely on a 0-10 scale
                    weight = weight / 10.0
                    logger.debug(f"Converted 0-10 scale weight {original_weight} to 0-1 scale: {weight}")
                    validation_stats["weight_corrections"] += 1
                else:
                    logger.warning(f"Weight {original_weight} out of range, using default 0.5")
                    validation_stats["invalid_weights"] += 1
            ...
            if weight < min_threshold:
                logger.debug(f"Applied minimum threshold for {rel_type}: {weight} -> {min_threshold}")
            ...
            if isinstance(keywords, str):
                ...
                logger.debug(f"Split keyword string '{original_keywords}' into list: {keywords}")
                validation_stats["keyword_fixes"] += 1
            elif not isinstance(keywords, list):
                keywords = []
                logger.debug(f"Converted non-list keywords {type(original_keywords)} to empty list")
                validation_stats["keyword_fixes"] += 1
            ...
            processed_rel = {
                ...
                "weight": weight,
                "description": rel.get("description", f"Relationship between {rel['source']} and {rel['target']}"),
                "keywords": keywords,
                "confidence": rel.get("confidence", weight),  # Default to weight if not provided
        ...
        logger.info(f"Relationship processing completed: {len(processed)}/{len(relationships)} relationships validated successfully")
        
        if any(validation_stats.values()):
            stats_summary = ", ".join([f"{k}: {v}" for k, v in validation_stats.items() if v > 0])
    ...
    async def enrich_relationship_metadata(
        ...
        if not all(k in relationship for k in ["source", "target", "relationship_type"]):
            return relationship
