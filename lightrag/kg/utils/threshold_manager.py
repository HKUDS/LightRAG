...
class ThresholdManager:
    ...
    def add_weight(self, weight: float, relationship_type: Optional[str] = None) -> None:
        """
        ...
        if self._samples_since_update >= self.update_interval and len(self._all_weights) >= self.min_samples:
    ...
    def _update_thresholds(self) -> None:
        ...
        if len(self._all_weights) >= self.min_samples:
            ...
            self._global_threshold = max(self.floor, min(self.ceiling, self._global_threshold))
            logger.info(f"Updated global threshold to {self._global_threshold:.3f} based on {len(self._all_weights)} samples")
        ...
        for rel_type, weights in self._type_weights.items():
            if len(weights) >= self.min_samples:
                ...
                logger.info(f"Updated threshold for '{rel_type}' to {threshold:.3f} based on {len(weights)} samples")
