...
class RelationshipFilterMetrics:
    ...
    def __init__(self, storage_path: Optional[str] = None):
        ...
        """
        self.storage_path = Path(storage_path) if storage_path else Path("./filter_metrics.json")
    ...
    def record_filter_session(self, filter_stats: Dict[str, Any], 
        ...
        session_data = {
            ...
            "retention_rate": filter_stats.get("total_after", 0) / max(filter_stats.get("total_before", 1), 1),
            "filter_breakdown": {
                ...
                "low_quality_relationships": filter_stats.get("low_quality_relationships", 0),
    ...
    def record_relationship_classification(self, relationship_type: str, category: str,
                                         confidence: float, was_kept: bool):
        """
    ...
    def get_session_summary(self) -> Dict[str, Any]:
        ...
        average_retention_rate = sum(s["retention_rate"] for s in sessions) / len(sessions)
        ...
        summary = {
            ...
            "overall_retention_rate": total_relationships_kept / max(total_relationships_processed, 1),
            "average_retention_rate": average_retention_rate,
            "filter_breakdown": dict(filter_breakdown),
            "classification_enabled": any(s.get("classification_enabled", False) for s in sessions)
        }
        
        # Add classification summary if available
        if classifications:
            summary["classification_summary"] = self._summarize_classifications(classifications)
    ...
    def get_performance_analysis(self) -> Dict[str, Any]:
        ...
        category_analysis = {}
        enhanced_sessions = [s for s in sessions if s.get("classification_enabled", False)]
        ...
        if enhanced_sessions:
            ...
            for category, rates in category_performance.items():
                ...
                category_analysis[category] = {
                    ...
                    "trend": "stable" if len(rates) < 3 else self._calculate_trend(rates)
                }
        ...
        return {
            "summary_stats": {
                ...
                "retention_trend": self._calculate_trend(retention_trend) if len(retention_trend) >= 3 else "insufficient_data"
            },
    ...
    def _summarize_classifications(self, classifications: List[Dict[str, Any]]) -> Dict[str, Any]:
        ...
        for rel_type, scores in confidence_scores.items():
            ...
            type_stats[rel_type]["retention_rate"] = type_stats[rel_type]["kept"] / type_stats[rel_type]["total"]
        
        # Find problematic types
        problematic_types = [
            rel_type for rel_type, stats in type_stats.items()
    ...
    def _generate_recommendations(self, sessions: List[Dict], category_analysis: Dict) -> List[str]:
        """Generate actionable recommendations based on performance data."""
        recommendations = []
        
        # Overall retention analysis
        recent_retention = sum(s["retention_rate"] for s in sessions[-5:]) / min(len(sessions), 5)
        ...
        if sessions:
            ...
            if total_filtered > 0:
                ...
                dominant_filters = [
                    (filter_type, count) for filter_type, count in filter_breakdown.items()
    ...
    def _update_aggregates(self, session_data: Dict[str, Any]):
        ...
        self.aggregate_metrics["total_sessions"] = self.aggregate_metrics.get("total_sessions", 0) + 1
        self.aggregate_metrics["total_relationships_processed"] = self.aggregate_metrics.get("total_relationships_processed", 0) + session_data["total_before"]
        self.aggregate_metrics["total_relationships_kept"] = self.aggregate_metrics.get("total_relationships_kept", 0) + session_data["total_after"]
        ...
        try:
            if self.storage_path.exists():
                with open(self.storage_path, 'r') as f:
                    data = json.load(f)
                    self.session_metrics = defaultdict(list, data.get("session_metrics", {}))
                    self.aggregate_metrics = defaultdict(lambda: defaultdict(int), data.get("aggregate_metrics", {}))
    ...
    def export_metrics_report(self, output_path: Optional[str] = None) -> str:
        ...
        if output_path is None:
            output_path = f"filter_metrics_report_{datetime.now().strftime('%Y%m%d_%H%M%S')}.json"
        ...
        report = {
            "report_metadata": {
                ...
                "time_range": {
                    "start": self.session_metrics["filter_sessions"][0]["timestamp"] if self.session_metrics["filter_sessions"] else None,
                    "end": self.session_metrics["filter_sessions"][-1]["timestamp"] if self.session_metrics["filter_sessions"] else None
                }
            },
            "session_summary": self.get_session_summary(),
            "performance_analysis": self.get_performance_analysis(),
            "raw_data": {
                "sessions": self.session_metrics["filter_sessions"][-50:],  # Last 50 sessions
                "classifications": self.session_metrics["classifications"][-1000:]  # Last 1000 classifications
            }
        }
