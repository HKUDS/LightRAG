...
class Neo4JStorage(BaseGraphStorage):
            ...
            for database in (DATABASE, None):
                ...
                try:
                    async with self._driver.session(database=database) as session:
                        try:
                            ...
                            logger.info(f'[{self.workspace}] Connected to {database} at {URI}')
                            connected = True
                        except neo4jExceptions.ServiceUnavailable as e:
                            logger.error(f'[{self.workspace}] ' + f'Database {database} at {URI} is not available')
                            raise e
                except neo4jExceptions.AuthError as e:
                    logger.error(f'[{self.workspace}] Authentication failed for {database} at {URI}')
                    raise e
                    ...
                    if e.code == 'Neo.ClientError.Database.DatabaseNotFound':
                        ...
                        try:
                            async with self._driver.session() as session:
                                ...
                                await result.consume()  # Ensure result is consumed
                                logger.info(f'[{self.workspace}] ' + f'Database {database} at {URI} created')
                                connected = True
                        except (
                            ...
                            if (
                                e.code == 'Neo.ClientError.Statement.UnsupportedAdministrationCommand'
                            ...
                            if database is None:
                                logger.error(f'[{self.workspace}] Failed to create {database} at {URI}')
                                raise e
                ...
                if connected:
                    ...
                    try:
                        ...
                        logger.warning(f'[{self.workspace}] Failed to create B-Tree index: {e!s}')
    ...
    async def _create_fulltext_index(self, driver: AsyncDriver, database: str, workspace_label: str):
        ...
        try:
            async with driver.session(database=database) as session:
                ...
                if existing_index:
                    ...
                    logger.info(f"[{self.workspace}] Found existing index '{index_name}' with state: {index_state}")
                    ...
                    logger.info(f"[{self.workspace}] No existing index '{index_name}' found. Creating new index.")

                # Create or recreate the index if needed
                needs_recreation = existing_index is not None and existing_index.get('state') != 'ONLINE'
                ...
                if needs_recreation or needs_creation:
                    ...
                    if needs_recreation:
                        try:
                            ...
                            logger.warning(f'[{self.workspace}] Failed to drop existing index: {drop_error!s}')
            ...
            if 'Unknown command' in str(e) or 'invalid syntax' in str(e).lower():
                ...
                logger.error(f"[{self.workspace}] Failed to create or verify full-text index '{index_name}': {e!s}")
    ...
    async def has_node(self, node_id: str) -> bool:
        ...
        async with self._driver.session(database=self._DATABASE, default_access_mode='READ') as session:
            ...
            try:
                ...
                logger.error(f'[{self.workspace}] Error checking node existence for {node_id}: {e!s}')
    ...
    async def get_node(self, node_id: str) -> dict[str, str] | None:
        ...
        async with self._driver.session(database=self._DATABASE, default_access_mode='READ') as session:
            try:
                ...
                try:
                    ...
                    if records:
                        ...
                        if 'labels' in node_dict:
                            node_dict['labels'] = [label for label in node_dict['labels'] if label != workspace_label]
                ...
                logger.error(f'[{self.workspace}] Error getting node for {node_id}: {e!s}')
                raise
        ...
        async with self._driver.session(database=self._DATABASE, default_access_mode='READ') as session:
            try:
                ...
                try:
                    ...
                    if not record:
                        logger.warning(f"[{self.workspace}] No node found with label '{node_id}'")
                        return 0
                ...
                logger.error(f'[{self.workspace}] Error getting node degree for {node_id}: {e!s}')
                raise
    ...
    async def get_edge(self, source_node_id: str, target_node_id: str) -> dict[str, str] | None:
        ...
        try:
            ...
            logger.error(f'[{self.workspace}] Error in get_edge between {source_node_id} and {target_node_id}: {e!s}')
            raise
        ...
        try:
            async with self._driver.session(database=self._DATABASE, default_access_mode='READ') as session:
                ...
                try:
                    ...
                    logger.error(f'[{self.workspace}] Error getting edges for node {source_node_id}: {e!s}')
                    if results is not None:
                        await results.consume()  # Ensure results are consumed even on error
                    raise
        except Exception as e:
            logger.error(f'[{self.workspace}] Error in get_node_edges for {source_node_id}: {e!s}')
            raise
        ...
        async with self._driver.session(database=self._DATABASE, default_access_mode='READ') as session:
            try:
                if node_label == '*':
                    ...
                    count_query = f'MATCH (n:`{workspace_label}`) RETURN count(n) as total'
                    count_result = None
                    try:
                    ...
                    try:
                        ...
                        if not full_record:
                            logger.debug(f'[{self.workspace}] No nodes found for entity_id: {node_label}')
                            return result
                ...
                if node_label != '*':
                    logger.warning(f'[{self.workspace}] Neo4j: falling back to basic Cypher recursive search...')
                    return await self._robust_fallback(node_label, max_depth, max_nodes)
                else:
                    logger.warning(
    ...
    async def _robust_fallback(self, node_label: str, max_depth: int, max_nodes: int) -> KnowledgeGraph:
        ...
        while queue and len(visited_nodes) < max_nodes:
            ...
            if current_depth > max_depth:
                logger.debug(f'[{self.workspace}] Skipping node at depth {current_depth} (max_depth: {max_depth})')
                continue
            ...
            if len(visited_nodes) >= max_nodes:
                result.is_truncated = True
                logger.info(f'[{self.workspace}] Graph truncated: breadth-first search limited to: {max_nodes} nodes')
                break
                ...
                for record in records:
                    ...
                    if edge_id not in visited_edges:
                        ...
                        if target_id:  # Only process if target node has entity_id
                            ...
                            if sorted_pair not in visited_edge_pairs and (
                                target_id in visited_nodes
                                or (target_id not in visited_nodes and current_depth < max_depth)
                            ):
    ...
    async def remove_edges(self, edges: list[tuple[str, str]]):
        ...
        for source, target in edges:
            ...
            async def _do_delete_edge(tx: AsyncManagedTransaction, source: str = source, target: str = target):
                workspace_label = self._get_workspace_label()
                query = f"""
                ...
                logger.debug(f"[{self.workspace}] Deleted edge from '{source}' to '{target}'")
                await result.consume()  # Ensure result is fully consumed
        ...
        try:
            ...
            async with self._driver.session(database=self._DATABASE, default_access_mode='READ') as session:
                if is_chinese:
                    ...
                    RETURN label
                    """
                    result = await session.run(cypher_query, query_strip=query_strip, limit=limit)
                else:
                    ...
                    RETURN label
                    """
                    result = await session.run(cypher_query, query_lower=query_lower, limit=limit)
