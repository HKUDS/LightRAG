...
def validate_relationship_schema(rel: Dict[str, Any]) -> bool:
    ...
    if not isinstance(rel.get("rel_type"), str) or not isinstance(rel.get("description"), str):
        return False
...
def normalize_relationship_data(rel: Dict[str, Any], original_rel: Dict[str, Any]) -> Dict[str, Any]:
    """
    ...
    for edge_key, edge_list in original_edges.items():
        for original_rel in edge_list:
            ...
            if lookup_key in validated_lookup:
                ...
                normalized_rel = normalize_relationship_data(validated_rel, original_rel)
                result_edges[edge_key].append(normalized_rel)
                
                # Determine if modified and log changes if requested
                weight_changed = abs(float(original_rel["weight"]) - float(validated_rel["weight"])) > 0.05
                description_changed = original_rel["description"] != validated_rel["description"]
                ...
                if weight_changed or description_changed:
                    ...
                    if log_changes:
                        changes = []
                        if weight_changed:
                            changes.append(f"weight: {original_rel['weight']:.2f} → {validated_rel['weight']:.2f}")
                        if description_changed:
                            changes.append(f"description: '{original_rel['description'][:50]}...' → '{validated_rel['description'][:50]}...'")
                        reason = validated_rel.get("reason", "No reason provided")
                        logger.info(f"Chunk {chunk_key}: Modified {original_rel['src_id']} → {original_rel['tgt_id']}: {', '.join(changes)} (Reason: {reason})")
                else:
                    stats["kept"] += 1
            else:
                # Relationship was removed
                stats["removed"] += 1
                logger.debug(f"Removed relationship: {original_rel['src_id']} -> {original_rel['tgt_id']}")
    
    logger.info(f"Chunk {chunk_key}: Kept {stats['kept']}, Modified {stats['modified']}, Removed {stats['removed']}")
    return result_edges
...
async def _post_process_chunk_relationships(
    ...
    if total_relationships > max_batch_size:
        logger.warning(f"Chunk {chunk_key}: {total_relationships} relationships exceed batch size {max_batch_size}, skipping validation")
        return maybe_edges
    ...
    try:
        ...
        for rel in validated_relationships:
            if validate_relationship_schema(rel):
                ...
                if not chunk_entity_names or (rel["src_id"] in chunk_entity_names and rel["tgt_id"] in chunk_entity_names):
                    valid_relationships.append(rel)
                else:
                    logger.debug(f"Chunk {chunk_key}: Skipping relationship with unknown entities: {rel['src_id']} -> {rel['tgt_id']}")
            else:
        ...
        logger.warning(f"Chunk {chunk_key}: Validation timed out after {timeout}s, using original relationships")
        return maybe_edges
...
def cleanup_orphaned_entities(
    ...
    if removed_count > 0:
        logger.info(f"Entity cleanup: Kept {kept_count}, Removed {removed_count} orphaned entities")
    else:
