...
class QueryLogger:
    ...
    def _build_log_entry(
        ...
        entry = {
            ...
            "query": query_text,
            "response": response_text[:500] if self.log_level == LogLevel.MINIMAL else response_text,
        }
    ...
    async def get_statistics(self) -> Dict[str, Any]:
        ...
        async with self._lock:
            ...
            if self.log_file_path.exists():
                stats["current_file_size_mb"] = self.log_file_path.stat().st_size / (1024 * 1024)
                stats["current_file_path"] = str(self.log_file_path)
            
            # Count backup files
            backup_count = sum(
                1 for i in range(1, self.backup_count + 1)
            ...
            if self.archive_dir and self.archive_dir.exists():
                stats["archived_files_count"] = len(list(self.archive_dir.glob("query_log_*.log.gz")))
                    ...
                    for line in f:
                        ...
                        try:
                            ...
                            if not self._matches_filters(
                                entry, search_term, user_id, session_id,
                                start_date, end_date, include_errors_only
                            ):
                                continue
