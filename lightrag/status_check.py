...
def print_system_health(health_data: Dict[str, Any]):
    ...
    if current:
        ...
        print(f"{memory_status} Memory Usage: {memory:.1f}% ({format_bytes(memory_used * 1024 * 1024)} used)")
...
def print_performance_stats(perf_data: Dict[str, Any]):
    ...
    sorted_ops = sorted(perf_data.items(), key=lambda x: x[1].get("count", 0), reverse=True)
    ...
    for op_name, stats in sorted_ops:
        ...
        print(f"   Range: {format_duration(min_duration)} - {format_duration(max_duration)}")
...
def print_processing_stats(proc_data: Dict[str, Any]):
    ...
    print(f"\n📈 Extraction Results:")
    if entities:
        print(f"   🔖 Entities: {entities.get('validated', 0):,} validated "
              f"({entities.get('success_rate', 0):.1f}% success rate)")
    if relationships:
        print(f"   🔗 Relationships: {relationships.get('validated', 0):,} validated "
              f"({relationships.get('success_rate', 0):.1f}% success rate)")
    if chunks:
        print(f"   📄 Chunks: {chunks.get('processed', 0):,} processed "
...
def monitor_system(duration: int, interval: int = 10):
    ...
    try:
        while time.time() - start_time < duration:
            ...
            cpu_status = "🔴" if current_health.cpu_percent > 90 else "🟡" if current_health.cpu_percent > 70 else "🟢"
            memory_status = "🔴" if current_health.memory_percent > 90 else "🟡" if current_health.memory_percent > 70 else "🟢"
...
def main():
    ...
    parser.add_argument("--monitor", "-m", type=int, help="Monitor system for N seconds")
