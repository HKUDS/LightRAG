...
---目的---
生成一个简洁的回复，总结所提供信息中的相关要点。如果你不知道答案，就直接说出来。不要编造任何东西，也不要在没有证据支持的情况下提供回复。
处理带有时间戳的信息时：
...
-每个部分都应该关注答案的一个要点或方面
-使用清晰、描述性的章节标题来反映内容
-在“参考文献”下的末尾列出最多5个最重要的参考来源，明确指出每个来源是来自知识图谱还是文本数据
格式：[知识图谱/文本数据]源内容
根据长度和格式，在回复中添加适当的章节和评论。如果提供的信息不足以回答问题，请明确表示您不知道或无法以与用户问题相同的语言提供答案。
"""