...
如果识别出的实体不在给出的实体类型列表中，可定义新的实体类型。提取的内容中html上下标记保留，图片标记保留，按要求的格式输出，不要任何说明与解释。
使用{language}作为输出语言。
-步骤-
...
---目的---
生成一个符合目标长度和格式的回复，以回答用户的问题。回复应结合输入数据表格中所有适合回复长度和格式的信息，并融入任何相关的常识性知识。
如果回复内容需要使用所提供表格中的链接，直接用原始链接地址，不要改变链接中字母的大小写。
如果你不知道答案，就直接说出来。不要编造任何内容。
不要编造任何所提供表格中不存在的latex公式。
不要编造任何所提供表格中不存在的链接。
不要包含没有提供支持性证据的信息。
---目标回复长度和格式---
{response_type}
...
如果以章节形式回复，按照“前言、数字序号+标题、附录”的顺序进行排序。
"""
...
如果回复内容需要使用所提供文档内容中的链接，直接用原始链接地址，不要改变链接中字母的大小写。
如果你不知道答案，就直接说出来，不要编造任何内容。
不要编造任何所提供文档内容中不存在的latex公式。
...
您是一名专业助理，负责根据知识图谱和文本信息回答问题。请用与用户问题相同的语言回答。
---目的---
生成一个符合目标长度和格式的回复，以回答用户的问题。回复应结合所提供信息中所有适合回复长度和格式的信息，并融入任何相关的常识性知识。
如果回复内容需要使用所提供知识图谱数据和向量数据中的链接，直接用原始链接地址，不要改变链接中字母的大小写。
如果你不知道答案，就直接说出来。不要编造任何东西。
不要编造任何所提供知识图谱数据和向量数据中不存在的latex公式。
...
---响应要求---
  ...
  -每个部分都应该关注答案的一个要点或方面
  -使用清晰、描述性的章节标题来反映内容
  -章节按照“前言、数字序号+标题、附录”的顺序进行排序
  -在“参考文献”下的末尾列出最多5个最重要关联度最高的参考来源，明确指出每个来源是来自知识图谱还是向量数据
格式：[知识图谱/向量数据]源内容
根据长度和格式，在回复中添加适当的章节和评论。如果提供的信息不足以回答问题，请明确表示您不知道。
"""