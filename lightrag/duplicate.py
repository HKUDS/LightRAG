...
class LLMBasedCleaning(BaseDeduplicationStrategy):
    ...
    async def classify_nodes_by_similarity(self, node_data) -> list:
        ...
        logger.info(f"Classifying nodes by similarity with batch size: {self.param.get('clean_split_batch_size', 30)}")
        TARGET_SIZE = self.param.get("clean_split_batch_size", 30)
        MAX_SIZE = int(TARGET_SIZE * 1.25)
        MIN_SIZE = int(TARGET_SIZE * 0.75)
        
        batcher_labels = SemanticClusterBatcher(TARGET_SIZE, MAX_SIZE, MIN_SIZE, self.param.get("clean_split_batch_model", "paraphrase-multilingual-MiniLM-L12-v2"))
        # Classify node data
    ...
    async def clean_nodes(
        ...
        for i in range(0, len(nodes_batches)):
            logger.info("#############################################################################")
            logger.info(f"                  ####  CLEANING NODES[{str(i + 1)}/{str(len(nodes_batches))}]  Length [{str(len(nodes_batches[i]))}]  ####")
            logger.info("#############################################################################")
            
            this_batch = nodes_batches[i]
            use_model_func: callable = self.rag.llm_model_func
            use_model_func = partial(use_model_func, _priority=5)
            clean_system_prompt = self.param.get('clean_system_prompt') or PROMPTS["goal_clean"]
            clean_system_prompt = str(clean_system_prompt) + '\n' + PROMPTS["goal_clean_examples"]
            ...
            if response is None or response.strip() in ["null", "", "[]", "Null", "NULL"]:
            ...
            if merge_operations:
                for op in merge_operations:
                    ...
                    if len(nodes_to_merge) == 1:
                        logger.info(f"    ———— Single output, skipping merge: {nodes_to_merge}")
                        continue
                    ...
                    if len(found_nodes) >= 2 and summarized_node != found_nodes:
                        ...
                        logger.info(f"    Merged nodes: {summarized_node} ← {found_nodes}")
                        await self.rag.amerge_entities(
                            source_entities=found_nodes, 
                            target_entity=summarized_node
                        )
                        # Remove merged nodes
                        ...
                        logger.info(f"    ———— Single output, skipping merge: {found_nodes}")
            else:
                logger.info("    No merge operations found")
        
        logger.info("################################################################################")
        logger.info(f"                     ####  NODE CLEANING COMPLETE  ####")
        logger.info("################################################################################")