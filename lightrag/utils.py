...
try:
    ...
    pypinyin = None  # type: ignore
    _PYPINYIN_AVAILABLE = False
    logger.warning('pypinyin is not installed. Chinese pinyin sorting will use simple string sorting.')
    ...
    def final_decro(func):
        ...
        if llm_timeout is not None:
            ...
            if max_execution_timeout is None:
                max_execution_timeout = llm_timeout * 2  # Reserved timeout buffer for low-level retry
            if max_task_duration is None:
                max_task_duration = llm_timeout * 2 + 15  # Reserved timeout buffer for health check phase
                ...
                while not shutdown_event.is_set():
                    try:
                        ...
                        try:
                            ...
                            if not task_state.future.done() and max_execution_timeout is not None:
                                task_state.future.set_exception(WorkerTimeoutError(max_execution_timeout, 'execution'))
                            ...
                            logger.error(f'{queue_name}: Error in decorated function for task {task_id}: {e!s}')
                            if not task_state.future.done():
        ...
        async def ensure_workers():
            ...
            async with initialization_lock:
                ...
                if active_tasks_count > 0 and reinit_count > 1:
                    logger.warning(f'{queue_name}: {active_tasks_count} tasks still running during reinitialization')
        ...
        async def wait_func(*args, _priority=10, _timeout=None, _queue_timeout=None, **kwargs):
            ...
            try:
                ...
                try:
                    ...
                    raise TimeoutError(f'{queue_name}: {e!s}') from e
        ...
        wait_func.shutdown = shutdown  # type: ignore
...
def exists_func(obj, func_name: str) -> bool:
    ...
    :return: True / False
    """
    return bool(callable(getattr(obj, func_name, None)))
    ...
    try:
        ...
        if chunk_data:
            ...
            if new_keys:
                ...
                logger.debug(f'Updated chunk {chunk_id} with {len(new_keys)} cache keys ({cache_scenario})')
    except Exception as e:
        logger.warning(f'Failed to update chunk {chunk_id} with cache references on {cache_scenario}: {e}')
...
def pick_by_weighted_polling(
    ...
    for i in range(n):
        ...
        expected_counts.append(round(expected))
...
def merge_source_ids(existing_ids: Iterable[str] | None, new_ids: Iterable[str] | None) -> list[str]:
