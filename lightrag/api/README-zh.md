...
`user_prompt` 是答案阶段的附加指令；结构化 Prompt 覆盖是另一类能力，用于覆盖查询执行时使用的提示词模板。如果自定义答案模板包含 `{user_prompt}`，系统会直接在该占位符处注入；否则 LightRAG 会自动追加一段标准的附加指令块。

- SDK 实例默认值：`LightRAG(prompt_config=...)`
- SDK 单次请求覆盖：`QueryParam(prompt_overrides=...)`
- API 请求字段：`prompt_overrides`

API 安全开关：

- `ALLOW_PROMPT_OVERRIDES_VIA_API` 默认值是 `false`
- 只有设置 `ALLOW_PROMPT_OVERRIDES_VIA_API=true` 后，API 才接受请求级 `prompt_overrides`
- `/health` 会暴露 `configuration.allow_prompt_overrides_via_api`，客户端可据此发现能力是否开启
...
```json
{
  "query": "什么是 LightRAG？",
  "mode": "mix",
  "user_prompt": "请使用简短要点回答",
  "prompt_overrides": {
    "query": {
      "rag_response": "请使用编号步骤回答。\n\n{context_data}\n\n{response_type}"
    }
  }
}
```
