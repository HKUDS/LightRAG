    ...
    def from_env(cls) -> "PasswordPolicy":
        ...
        return cls(
            ...
            lockout_duration_minutes=int(
                os.getenv("PASSWORD_LOCKOUT_DURATION_MINUTES", "30")
            ),
            complexity_score_threshold=int(
                os.getenv("PASSWORD_COMPLEXITY_THRESHOLD", "3")
            ),
        )
...
class PasswordManager:
    ...
    async def check_password_history(
        self, user_id: str, new_password: str, db_connection
    ) -> bool:
        """
    ...
    async def store_password_history(
        self, user_id: str, password_hash: str, db_connection
    ) -> None:
        """
    ...
    async def check_account_lockout(
        self, user_id: str, db_connection
    ...
    async def record_failed_attempt(self, user_id: str, db_connection) -> bool:
        ...
        try:
            ...
            user_data = await db_connection.fetchrow(
                "SELECT password_attempts FROM users WHERE id = ?", user_id
            )
            ...
            if (
                ...
                logger.warning(
                    f"Account locked for user {user_id} due to {self.policy.max_failed_attempts} failed attempts"
                )
                return True
