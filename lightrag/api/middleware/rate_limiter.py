...
class RedisRateLimitStore:
    ...
    async def record_violation(self, ip_address: str, endpoint: str, user_agent: str = "") -> None:
        ...
        try:
            ...
            await self._redis.zadd(
                violation_key, 
                {json.dumps(violation_data): current_time}
            )
    ...
    async def check_ip_block(self, ip_address: str, threshold: int, window_minutes: int) -> Tuple[bool, Optional[datetime]]:
        """
        ...
        try:
            ...
            violation_count = await self._redis.zcount(violation_key, window_start, time.time())
            ...
            if violation_count >= threshold:
                ...
                logger.warning(f"IP {ip_address} blocked due to {violation_count} violations")
    ...
    async def check_ip_block(self, ip_address: str, threshold: int, window_minutes: int) -> Tuple[bool, Optional[datetime]]:
        ...
        async with self._lock:
            ...
            if ip_address in self._violations:
                ...
                recent_violations = [t for t in self._violations[ip_address] if t > window_start]
                ...
                if len(recent_violations) >= threshold:
                    ...
                    logger.warning(f"IP {ip_address} blocked due to {len(recent_violations)} violations")
...
class AdvancedRateLimiter:
    ...
    async def initialize(self):
        ...
        if REDIS_AVAILABLE and self.config.redis_url:
            try:
                ...
                logger.warning(f"Redis initialization failed, using in-memory store: {e}")
    ...
    async def check_rate_limit(self, request: Request, limit_type: RateLimitType) -> Tuple[bool, Dict[str, Any]]:
        """
        ...
        if self.config.enable_ip_blocking:
            ...
            if is_blocked:
                return False, {
                    ...
                    "retry_after": int((unblock_time - datetime.utcnow()).total_seconds()) if unblock_time else 3600
                }
        
        # Get rate limit configuration
        rate_limit_str = self._rate_limits.get(limit_type, self.config.general_api_limit)
        limit, window_seconds = self._parse_rate_limit(rate_limit_str)
        
        # Generate rate limit key
        client_id = self._get_client_identifier(request)
        rate_limit_key = f"{self.config.redis_key_prefix}limit:{limit_type.value}:{client_id}"
...
class RateLimitMiddleware(BaseHTTPMiddleware):
    ...
    async def dispatch(self, request: Request, call_next):
        ...
        if rate_limit_info:
            ...
            if rate_limit_info.get("warning_mode"):
                response.headers["X-RateLimit-Warning"] = "Rate limit exceeded (warning mode)"
    ...
    def _determine_limit_type(self, request: Request) -> RateLimitType:
        ...
        if any(auth_path in path for auth_path in ["/login", "/auth", "/token", "/logout"]):
            return RateLimitType.AUTHENTICATION
        
        # Document operations
        if any(doc_path in path for doc_path in ["/documents/upload", "/documents/batch"]):
        ...
        # Graph operations
        if any(graph_path in path for graph_path in ["/graph", "/entities", "/relationships"]):
