...
class SecurityHeadersConfig:
    ...
    def from_env(cls) -> "SecurityHeadersConfig":
        ...
        return cls(
            ...
            enable_permissions_policy=get_env_bool(
...
class SecurityHeadersMiddleware(BaseHTTPMiddleware):
    ...
    def _add_content_specific_headers(self, request: Request, response: Response):
        ...
        if any(
            path in request.url.path.lower() for path in ["/auth", "/login", "/admin"]
        ):
    ...
    def analyze_security_posture(self) -> Dict[str, any]:
        ...
        if self.config.enable_csp:
            ...
            if "'unsafe-inline'" in self.config.csp_script_src:
                ...
                analysis["recommendations"].append(
                    "Remove 'unsafe-inline' from script-src for better security"
                )
        ...
        if self.config.enable_hsts:
            ...
            if self.config.hsts_max_age < 31536000:  # 1 year
                analysis["recommendations"].append(
                    "Increase HSTS max-age to at least 1 year"
                )
        ...
        if self.config.enable_x_headers:
            ...
            analysis["recommendations"].append(
                "Enable X-Security headers for legacy browser support"
            )
        ...
        if self.config.x_frame_options == "DENY":
            ...
            analysis["score"] += 5
            analysis["recommendations"].append(
                "Consider using DENY for X-Frame-Options for stronger protection"
            )
        ...
        if self.config.enable_permissions_policy:
            ...
            analysis["recommendations"].append(
                "Configure Permissions Policy to control browser features"
            )
        ...
        if self.config.hide_server_header:
            ...
            analysis["recommendations"].append(
                "Hide server header to reduce information disclosure"
            )
