...
try:
    ...
    print("Warning: Could not import PasswordManager. Password migration will be skipped.")
...
class AuthPhase1Migration:
    ...
    async def _enhance_users_table(self):
        ...
        columns_to_add = [
            ...
            ("security_questions_set", "BOOLEAN DEFAULT FALSE", "Security questions configured"),
    ...
    async def _migrate_existing_passwords(self):
        ...
        if not self.password_manager:
            logger.warning("Password manager not available, skipping password migration")
            return
        ...
        logger.info(f"Password migration completed: {migrated_count} migrated, {failed_count} failed")
    ...
    async def _log_migration_success(self):
        ...
        """, self.migration_name, self.version)
    ...
    async def _log_migration_failure(self, error_message: str):
        ...
        await self.db.execute("""
            ...
            WHERE migration_name = ? AND status = 'started'
        """, error_message, self.migration_name)
    ...
    async def _rollback_users_enhancements(self):
        ...
        for column in columns_to_remove:
            try:
                await self.db.execute(f"ALTER TABLE users DROP COLUMN IF EXISTS {column}")
...
if __name__ == "__main__":
    ...
    parser = argparse.ArgumentParser(description="LightRAG Authentication Phase 1 Migration")
