    ...
    try:
        ...
        try:
            ...
            try:
                ...
                logger.error(
                    f"Cannot decode file {file_path.name} - unsupported encoding"
                )
                return ""
        ...
        if content.startswith("b'") or content.startswith('b"'):
            logger.error(
                f"File {file_path.name} appears to contain binary data representation"
            )
            return ""
...
async def enhanced_pipeline_process_any_file(
    ...
    try:
        ...
        logger.error(
            f"Error in enhanced pipeline for {file_path.name}: {e}", exc_info=True
        )
        return False, track_id or ""
