...
class DoclingServiceProcessor:
        ...
        try:
            ...
            if result.success:
                ...
                return {
                    ...
                    "success": False,
                    "error": result.error_message or "Docling service processing failed"
                }
            ...
            logger.error(f"Unexpected error processing {file_path.name} with Docling service: {e}")
            return {
                "content": "",
                "metadata": {
                    "processor": "docling_service",
    ...
    async def process_document(self, file_path: Path) -> Dict[str, Any]:
        ...
        logger.debug(f"Processing strategy for {file_path.name}: "
        ...
        if use_service:
            ...
            if service_result["success"]:
                logger.info(f"Successfully processed {file_path.name} with Docling service")
                return service_result
            else:
                # Service failed, try fallback if enabled
                logger.warning(f"Docling service failed for {file_path.name}, trying fallback")
                ...
                if fallback_enabled:
                    ...
                    fallback_result["metadata"]["docling_service_error"] = service_result.get("error")
                    
                    return fallback_result
                else:
                    logger.error(f"Docling service failed and fallback disabled for {file_path.name}")
                    return service_result  # Return the service error
        else:
            # Use fallback processor directly
            logger.info(f"Using fallback processor for {file_path.name} (reason: {reason})")
            fallback_result = await self.process_with_fallback(file_path)
