...
class AuditLogger:
    ...
    async def log_auth_event(self, event_type: AuditEventType, user_id: str = None,
    ...
    async def _check_alerts(self, event: AuditEvent):
        ...
        if event.event_type == AuditEventType.LOGIN_FAILURE:
            ...
            if recent_failures >= 5:
                await self._send_alert(event, f"Multiple login failures: {recent_failures}")
    ...
    def get_recent_failures(self, identifier: str, minutes: int = 15) -> int:
        ...
        for event in self._recent_events:
            if (event.timestamp > cutoff and
                event.event_type in failure_events and
