        ...
        if response.status != 200:
            ...
            if is_html_error:
                if response.status == 502:
                    clean_error = (
                        'Bad Gateway (502) - Rerank service temporarily unavailable. Please try again in a few minutes.'
                    )
                elif response.status == 503:
                    clean_error = (
                        'Service Unavailable (503) - Rerank service is temporarily overloaded. Please try again later.'
                    )
