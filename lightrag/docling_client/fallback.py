...
class BasicDocumentProcessor:
    ...
    async def process_document(self, file_path: Path, **kwargs) -> Dict[str, Any]:
        ...
        logger.info(
            f"Processing {file_path.name} with basic parser (Docling service unavailable)"
        )
        ...
        try:
            ...
            metadata_section = self._generate_basic_metadata(
                file_path, content, processing_time, extension
            )
            ...
            logger.info(
                f"Basic processing completed for {file_path.name} "
    ...
    def _generate_basic_metadata(
        self, file_path: Path, content: str, processing_time: float, extension: str
    ) -> str:
        ...
        lines.append("- **Processor**: Basic Parser (Docling service unavailable)")
...
class FallbackProcessor:
    ...
    async def process_document_with_fallback(
        self, file_path: Path, docling_service_available: bool = False, **kwargs
    ) -> Dict[str, Any]:
        """
