...
class ServiceDiscovery:
    ...
    def __init__(self):
        ...
        self.health_check_interval: int = int(
            get_env_value("DOCLING_HEALTH_CHECK_INTERVAL", 30)
        )
    ...
    def get_service_url(self) -> Optional[str]:
        ...
        if self.service_url is None:
            ...
            self.service_url = (
                get_env_value("DOCLING_SERVICE_URL", None)
                or get_env_value("LIGHTRAG_DOCLING_SERVICE_URL", None)
                or get_env_value("DOCLING_HOST", None)
            )

            # If we got a host without protocol, add it
            if self.service_url and not self.service_url.startswith(
                ("http://", "https://")
            ):
    ...
    async def is_service_available(self, force_check: bool = False) -> bool:
        ...
        if (
            not force_check
            and self.service_available is not None
            and time.time() - self.last_health_check < self.health_check_interval
        ):
        ...
        try:
            ...
            async with httpx.AsyncClient(timeout=self.service_timeout) as client:
                ...
                if response.status_code == 200:
                    ...
                    service_healthy = health_data.get("status") in [
                        "healthy",
                        "degraded",
                    ]
                    ...
                    if service_healthy:
                        logger.debug(
                            f"Docling service is available at {service_url} "
                            f"(status: {health_data.get('status')})"
                        )
                    else:
                        logger.warning(
                            f"Docling service reports unhealthy status at {service_url} "
                            f"(status: {health_data.get('status')})"
                        )
                    ...
                    logger.warning(
                        f"Docling service health check failed at {health_url} "
        ...
        try:
            ...
            async with httpx.AsyncClient(timeout=self.service_timeout) as client:
                ...
                if response.status_code == 200:
                    ...
                    logger.warning(
                        f"Failed to get service config "
                        f"(status code: {response.status_code})"
                    )
                    return None
