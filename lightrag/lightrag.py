...
class LightRAG:
    ...
    vlm_llm_model_max_async: int | None = field(
        default_factory=lambda: int(os.environ["MAX_ASYNC_VLM_LLM"]) if "MAX_ASYNC_VLM_LLM" in os.environ else None
    )
    """Max concurrent VLM calls in analyze_multimodal."""

    extract_llm_model_max_async: int | None = field(
        default_factory=lambda: int(os.environ["MAX_ASYNC_EXTRACT_LLM"]) if "MAX_ASYNC_EXTRACT_LLM" in os.environ else None
    )
    """Max concurrent LLM calls for entity/relation extraction."""

    keyword_llm_model_max_async: int | None = field(
        default_factory=lambda: int(os.environ["MAX_ASYNC_KEYWORD_LLM"]) if "MAX_ASYNC_KEYWORD_LLM" in os.environ else None
    )
    """Max concurrent LLM calls for keyword extraction."""

    query_llm_model_max_async: int | None = field(
        default_factory=lambda: int(os.environ["MAX_ASYNC_QUERY_LLM"]) if "MAX_ASYNC_QUERY_LLM" in os.environ else None
    )
    ...
    def _get_effective_role_llm_timeout(self, role: str) -> int | None:
        return getattr(self, self._role_llm_timeout_attr(role))
    ...
    async def analyze_multimodal(
        ...
        try:
            ...
            sem = asyncio.Semaphore(max(1, self._get_effective_role_llm_max_async("vlm")))
            ...
            logger.warning(f"[analyze_multimodal] failed to update analyze_time for d-id: {doc_id}: {e}")
        return parsed_data
