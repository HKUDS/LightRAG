    ...
    async def infer_new_relationships(
        ...
        try:
            ...
            if not all_entities or len(all_entities) < 2:
                logger.info(
                    "Insufficient number of entities in the knowledge graph for relational reasoning"
                )
                return
            ...
            logger.info(
                f"We will perform relational inference on {len(filtered_entities)} entities"
            )

            kg_data = {}
            for entity in filtered_entities:
                subgraph = await self.get_knowledge_graph(
                    entity, max_depth=max_graph_depth
                )
                if subgraph:
            ...
            if not kg_data:
                logger.info(
                    "Unable to obtain knowledge graph data, unable to perform relational reasoning"
                )
                return

            prompt = PROMPTS["relation_inference"].format(kg_data=kg_data)

            if not self.llm_model_func:
                logger.warning(
                    "LLM model function not set, skipping relationship inference"
                )
                return
            ...
            try:
                ...
                if json_match:
                    ...
                    try:
                        ...
                        logger.warning(
                            f"Unable to parse large model response as JSON:{e}"
                        )
                        return response
                if not isinstance(new_relationships, list):
                    logger.warning(
                        f"The relationship format returned by the large model is incorrect:{new_relationships}"
                    )
                    return
                ...
                for rel in new_relationships:
                    try:
                        ...
                        if not bidirectional and reverse_key in processed_relations:
                            logger.info(
                                f"Skip processed reverse relationships:{target} -> {source}"
                            )
                            continue
                        ...
                        if not source_exists or not target_exists:
                            logger.warning(
                                f"Entity does not exist: {source} or {target}"
                            )
                            continue
                        ...
                        if existing_edge:
                            logger.info(
                                f"Relationship already exists: {source} -> {target}"
                            )
                            continue
                        ...
                        added_count += 1
                        logger.info(
                            f"Successfully added relationship: {source} -> {target},  Description: {description}"
                        )
            ...
            logger.error(
                f"Error occurred during the process of relational reasoning: {e}"
            )
            raise
