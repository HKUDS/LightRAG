...
class LightRAG:
    ...
    kg_chunk_pick_method: str = field(default=get_env_value('KG_CHUNK_PICK_METHOD', DEFAULT_KG_CHUNK_PICK_METHOD, str))
    ...
    entity_resolution_config: EntityResolutionConfig = field(default_factory=EntityResolutionConfig)
    ...
    orphan_connection_max_degree: int = field(default=get_env_value('ORPHAN_CONNECTION_MAX_DEGREE', 0, int))
    ...
    default_embedding_timeout: int = field(default=int(os.getenv('EMBEDDING_TIMEOUT', DEFAULT_EMBEDDING_TIMEOUT)))
    ...
    summary_max_tokens: int = field(default=int(os.getenv('SUMMARY_MAX_TOKENS', DEFAULT_SUMMARY_MAX_TOKENS)))
    """Maximum tokens allowed for entity/relation description."""

    summary_context_size: int = field(default=int(os.getenv('SUMMARY_CONTEXT_SIZE', DEFAULT_SUMMARY_CONTEXT_SIZE)))
    ...
    max_parallel_insert: int = field(default=int(os.getenv('MAX_PARALLEL_INSERT', DEFAULT_MAX_PARALLEL_INSERT)))
    """Maximum number of parallel insert operations."""

    max_graph_nodes: int = field(default=get_env_value('MAX_GRAPH_NODES', DEFAULT_MAX_GRAPH_NODES, int))
    ...
    max_file_paths: int = field(default=get_env_value('MAX_FILE_PATHS', DEFAULT_MAX_FILE_PATHS, int))
    ...
    def __post_init__(self):
        ...
        if self.force_llm_summary_on_merge < 3:
            logger.warning(f'force_llm_summary_on_merge should be at least 3, got {self.force_llm_summary_on_merge}')
        ...
        async with get_data_init_lock():
            try:
                ...
                try:
                    ...
                    for _checked_count, doc_id in enumerate(list(processed_docs.keys())[:max_check], 1):
    ...
    async def _migrate_entity_relation_data(self, processed_docs: dict):
        ...
        for doc_id, doc_status in processed_docs.items():
            chunk_ids = doc_status.chunks_list if hasattr(doc_status, 'chunks_list') and doc_status.chunks_list else []
        ...
        logger.info(f'Data migration completed: migrated {migration_count} documents with entities/relations')
        ...
        if need_entity_migration:
            ...
            for batch_idx in range(total_batches):
                ...
                for node in batch_nodes:
                    ...
                    chunk_ids = [chunk_id for chunk_id in raw_source.split(GRAPH_FIELD_SEP) if chunk_id]
                    if not chunk_ids:
                        continue
            ...
            if total_migrated > 0:
                ...
                logger.info(f'Entity chunk_tracking migration completed: {total_migrated} records persisted')
    ...
    async def get_knowledge_graph(
        ...
        max_nodes = self.max_graph_nodes if max_nodes is None else min(max_nodes, self.max_graph_nodes)
    ...
    async def _validate_and_fix_document_consistency(
        ...
        for doc_id, status_doc in to_process_docs.items():
            ...
            content_data = await self.full_docs.get_by_id(doc_id)
            if content_data and (
                hasattr(status_doc, 'status') and status_doc.status in [DocStatus.PROCESSING, DocStatus.FAILED]
            ):
    ...
    async def apipeline_process_enqueue_documents(
        ...
        try:
            ...
            while True:
                ...
                _first_doc_id, first_doc = next(iter(to_process_docs.items()))
                ...
                async def process_document(
                    ...
                    async with semaphore:
                        ...
                        try:
                            ...
                            all_tasks = first_stage_tasks + ([entity_relation_task] if entity_relation_task else [])
                        ...
                        if file_extraction_stage_ok:
                            try:
                                ...
                                if isinstance(e, PipelineCancelledException):
                                    # User cancellation - log brief message only, no traceback
                                    error_msg = (
                                        f'User cancelled during merge {current_file_number}/{total_files}: {file_path}'
                                    )
                                    logger.warning(error_msg)
                                    async with pipeline_status_lock:
                                        pipeline_status['latest_message'] = error_msg
    ...
    async def ainsert_custom_kg(
        ...
        try:
            ...
            all_chunks_data: dict[str, dict[str, str | int]] = {}
    ...
    async def aget_docs_by_ids(self, ids: str | list[str]) -> dict[str, dict[str, Any]]:
        ...
        results_list: list[dict[str, Any] | None] = await asyncio.gather(*tasks)

        # Build the result dictionary, mapping found IDs to their statuses
        found_statuses: dict[str, dict[str, Any]] = {}
    ...
    async def adelete_by_doc_id(self, doc_id: str, delete_llm_cache: bool = False) -> DeletionResult:
        ...
        **Concurrency Control Design:**
        ...
        async with pipeline_status_lock:
            if not pipeline_status.get('busy', False):
                ...
                pipeline_status['history_messages'][:] = [f'Starting deletion for document: {doc_id}']
            else:
        ...
        try:
            ...
            if doc_status != DocStatus.PROCESSED:
                if doc_status == DocStatus.PENDING:
                    ...
                    status_text = doc_status.value if isinstance(doc_status, DocStatus) else str(doc_status)
                    ...
                    try:
                        ...
                        for chunk_data in chunk_data_list:
                            ...
                            for cache_id in cache_ids:
                                if isinstance(cache_id, str) and cache_id and cache_id not in seen_cache_ids:
            ...
            try:
                ...
                if doc_relations_data and 'relation_pairs' in doc_relations_data:
                    ...
                    edges_dict = await self.chunk_entity_relation_graph.get_edges_batch(edge_pairs_dicts)
            ...
            try:
                ...
                for node_data in affected_nodes:
                    ...
                    if self.entity_chunks:
                        ...
                        if stored_chunks and isinstance(stored_chunks, dict):
                            existing_sources = [chunk_id for chunk_id in stored_chunks.get('chunk_ids', []) if chunk_id]

                    if not existing_sources and node_data.get('source_id'):
                        existing_sources = [
                            chunk_id for chunk_id in node_data['source_id'].split(GRAPH_FIELD_SEP) if chunk_id
                        ]
                ...
                for edge_data in affected_edges:
                    ...
                    if not existing_sources:
                        existing_sources = [
                            chunk_id for chunk_id in edge_data['source_id'].split(GRAPH_FIELD_SEP) if chunk_id
                        ]
            ...
            if entities_to_delete:
                try:
                    ...
                    if edges_still_exist:
                        logger.warning(f'⚠️ {edges_still_exist} entities still has edges before deletion')
                    ...
                    if edges_to_delete:
                        ...
                        # Delete from relation_chunks storage
                        if self.relation_chunks:
                            relation_storage_keys = [make_relation_chunk_key(src, tgt) for src, tgt in edges_to_delete]
                            await self.relation_chunks.delete(relation_storage_keys)
                    ...
                    entity_vdb_ids = [compute_mdhash_id(entity, prefix='ent-') for entity in entities_to_delete]
            ...
            if delete_llm_cache and doc_llm_cache_ids and self.llm_response_cache:
                try:
                    ...
                    cache_log_message = (
                        f'Successfully deleted {len(doc_llm_cache_ids)} LLM cache entries for document {doc_id}'
                    )
            ...
            if deletion_operations_started:
                try:
                    ...
                    persistence_error_msg = (
                        f'Failed to persist data after deletion attempt for {doc_id}: {persistence_error}'
                    )
    ...
    async def aconnect_orphan_entities(
        ...
        db = getattr(self.entities_vdb, 'db', None)
        workspace = getattr(self.entities_vdb, 'workspace', self.workspace)
        if db is None:
        ...
        try:
            ...
            if target_max_degree == 0:
                ...
                orphans = await db.query(sparse_sql, [workspace, target_max_degree], multirows=True)
            ...
            for orphan in orphans:
                ...
                for candidate in candidates:
                    ...
                    try:
                        ...
                        if not json_match:
                            logger.warning(
                                f'[{workspace}] No JSON in LLM response for {orphan_name} -> {candidate_name}'
                            )
                            continue
                        ...
                        if should_connect and confidence >= conf_threshold:
                            ...
                            try:
                                ...
                                result['errors'].append(
                                    f'Failed to create relation {orphan_name} -> {candidate_name}: {e!s}'
                                )
                        else:
                            logger.debug(
                        ...
                        result['errors'].append(f'JSON parse error for {orphan_name} -> {candidate_name}: {e!s}')
                    except Exception as e:
                        result['errors'].append(f'LLM validation error for {orphan_name} -> {candidate_name}: {e!s}')

        except Exception as e:
            result['errors'].append(f'Orphan connection failed: {e!s}')
    ...
    async def aprocess_orphan_connections_background(
        ...
        async with orphan_lock:
            if orphan_status.get('busy'):
                ...
                logger.info(f'[{self.workspace}] Orphan connection already running, request pending')
        ...
        try:
            ...
            workspace = getattr(self.entities_vdb, 'workspace', self.workspace)
            if db is None:
                await self._append_orphan_message('Error: Orphan connection requires PostgreSQL vector storage')
                result['errors'].append('Orphan connection requires PostgreSQL vector storage')
                return result
            ...
            if max_degree == 0:
                ...
                await self._append_orphan_message(f'Finding sparse entities (degree ≤ {max_degree})...')
                sparse_sql = SQL_TEMPLATES['get_sparse_entities']
                orphans = await db.query(sparse_sql, [workspace, max_degree], multirows=True)
            ...
            for i, orphan in enumerate(orphans):
                ...
                if orphan_status.get('cancellation_requested'):
                    await self._append_orphan_message('Cancellation requested, stopping...')
                    break
                ...
                for candidate in candidates:
                    ...
                    try:
                        if not self.llm_model_func:
                            logger.warning('LLM model function not initialized for orphan validation')
                            continue
                        llm_callable = cast(Callable[[str], Awaitable[str]], self.llm_model_func)
                        llm_response = await llm_callable(validation_prompt)
                        ...
                        result['errors'].append(f'LLM validation error for {orphan_name}: {e!s}')
            ...
            if errors:
                await self._append_orphan_message(f'Encountered {len(errors)} errors during processing')

        except Exception as e:
            error_msg = f'Orphan connection failed: {e!s}'
            cast(list, result['errors']).append(error_msg)
            ...
            async with orphan_lock:
                ...
                if orphan_status.get('request_pending'):
                    ...
                    await self._append_orphan_message('Note: Another request was pending during processing')
