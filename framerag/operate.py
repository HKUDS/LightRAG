...
# ─────────────────────────────────────────────────────────────────────────────
# Call 2 (legacy): Event Detection + Frame Induction + Role Assignment
# ─────────────────────────────────────────────────────────────────────────────
...
async def extract_events_frames_roles(
    ...
    def _resolve_filler(filler_text: str) -> Optional[EntityMentionSchema]:
        """Exact match first; fall back to longest-overlap substring match.
        Skips ambiguous cases where multiple mentions match equally."""
        key = filler_text.lower().strip()
        if key in mention_by_name:
            return mention_by_name[key]
        if len(key) < 3:
            return None
        # Collect all substring matches and pick longest overlap to avoid
        # "Mary" matching both "Mary Sutherland" and "Mary Holder" arbitrarily.
        candidates: list[tuple[int, EntityMentionSchema]] = []
        for name, m in mention_by_name.items():
            if key in name or name in key:
                overlap = len(set(key.split()) & set(name.split()))
    ...
    for item in raw:
        ...
        frame_name = _normalize_frame_name(frame_data.get("frame_name", "").strip())
        if not frame_name:
            continue

        lexical_unit = frame_data.get("lexical_unit", f"{trigger_lemma}.v")

        # Resolve participant entity mentions (with partial-name fallback)
        participant_mention_ids: list[str] = []
        for pname in item.get("participant_entity_names", []):
            matched = _resolve_filler(pname)
            if matched:
                participant_mention_ids.append(matched.mention_id)
        ...
        for fe_item in role_data.get("core", []):
            ...
            if filler_type_raw == "ENTITY":
                matched = _resolve_filler(str(filler_text))
                core_assignments.append(FEAssignment(
                    fe_name=fe_name,
                    filler_id=matched.mention_id if matched else None,
                    filler_type="ENTITY" if matched else "MISSING",
