...
class TestPerRoleConcurrencyAndTimeout:
    def test_none_role_values_fallback_to_base(self, tmp_path):
        ...
        # Test default instantiation (no explicit env vars or kwargs for role-specific max_async)
        # Assuming DEFAULT_MAX_ASYNC is something else, we pass explicit base llm_model_max_async
        # Wait, the best way to test this is just to create a LightRAG object and check its effective values.

        # We need to mock _make_rag to ensure no env vars pollute
        import os
        for k in list(os.environ.keys()):
            if k.startswith("MAX_ASYNC_") or k.startswith("LLM_TIMEOUT_"):
                del os.environ[k]

        def _dummy_embedding(*args, **kwargs):
            import numpy as np
            return np.zeros((len(args[0]), 8))
        ...
        rag = LightRAG(
            ...
            embedding_func=EmbeddingFunc(embedding_dim=8, max_token_size=8192, func=_dummy_embedding),
            llm_model_max_async=4,
            default_llm_timeout=180,
        )

        # Raw attributes should be None (fallback not yet applied)
        ...
        # Effective values should fall back to base values
        assert rag._get_effective_role_llm_max_async("extract") == 4
        assert rag._get_effective_role_llm_max_async("keyword") == 4
        assert rag._get_effective_role_llm_max_async("query") == 4

        # Effective timeouts should be None (to use SDK defaults)
        assert rag._get_effective_role_llm_timeout("extract") is None
        assert rag._get_effective_role_llm_timeout("keyword") is None
        assert rag._get_effective_role_llm_timeout("query") is None
