...
class TestPasswordManager:
    ...
    def test_password_policy_creation(self):
        ...
        custom_policy = PasswordPolicy(
            min_length=12, require_special_chars=False, history_count=10
        )
    ...
    def test_password_validation(self):
        ...
        for password, expected_valid, expected_strength in test_cases:
            ...
            ), f"Password '{password}' strength mismatch"

            if not expected_valid:
                assert (
                    len(errors) > 0
                ), f"Expected errors for invalid password '{password}'"
...
class TestRateLimiter:
    ...
    async def test_warning_mode(self):
        ...
        warning_config = RateLimitConfig(
            enabled=True, warning_mode=True, authentication_limit="1/minute"
        )
