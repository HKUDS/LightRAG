...
async def test_batch_document_dates_are_aligned_and_validated_atomically(tmp_path):
    ...
    try:
        ...
        with pytest.raises(ValueError, match="YYYY, YYYY-MM, or YYYY-MM-DD"):
            await rag.apipeline_enqueue_documents(
                input=["valid first", "invalid second"],
                ids=["doc-valid-first", "doc-invalid-second"],
                document_dates=["2018", "2018-02-30"],
            )

        assert await rag.full_docs.get_by_id("doc-valid-first") is None
        assert await rag.full_docs.get_by_id("doc-invalid-second") is None
    finally:
        await rag.finalize_storages()
