...
def check_env_file():
    ...
    if not os.path.exists('.env'):
        warning_msg = (
            'Warning: .env file not found in the current directory. This may affect storage configuration loading.'
        )
...
async def test_graph_basic(storage):
    ...
    try:
        ...
        if node1_props:
            ...
            assert node1_props.get('description') == node1_data['description'], 'Node description mismatch'
            assert node1_props.get('entity_type') == node1_data['entity_type'], 'Node type mismatch'
        else:
            print(f'Failed to read node properties: {node1_id}')
            pytest.fail(f'Failed to read node properties: {node1_id}')
        ...
        if edge_props:
            ...
            raise AssertionError(f'Failed to read edge properties: {node1_id} -> {node2_id}')
...
async def test_graph_advanced(storage):
    ...
    try:
        ...
        assert edge_degree == 3, f'Degree of edge {node1_id} -> {node2_id} should be 3, but got {edge_degree}'
        ...
        assert len(node2_edges) == 2, f'Node {node2_id} should have 2 edges, but got {len(node2_edges)}'
        ...
        assert has_connection_with_node1, f'Edge list of node {node2_id} should include a connection with {node1_id}'
        assert has_connection_with_node3, f'Edge list of node {node2_id} should include a connection with {node3_id}'
        print(
...
async def test_graph_batch_operations(storage):
    ...
    try:
        ...
        assert nodes_dict[node1_id]['description'] == node1_data['description'], f'{node1_id} description mismatch'
        assert nodes_dict[node2_id]['description'] == node2_data['description'], f'{node2_id} description mismatch'
        assert nodes_dict[node3_id]['description'] == node3_data['description'], f'{node3_id} description mismatch'
        ...
        assert len(node_degrees) == 3, f'Should return degrees of 3 nodes, but got {len(node_degrees)}'
        ...
        assert node_degrees[node1_id] == 3, f'Degree of {node1_id} should be 3, but got {node_degrees[node1_id]}'
        assert node_degrees[node2_id] == 2, f'Degree of {node2_id} should be 2, but got {node_degrees[node2_id]}'
        assert node_degrees[node3_id] == 3, f'Degree of {node3_id} should be 3, but got {node_degrees[node3_id]}'
        ...
        assert len(edge_degrees) == 3, f'Should return degrees of 3 edges, but got {len(edge_degrees)}'
        assert (
            node1_id,
            node2_id,
        ...
        assert len(nodes_edges) == 2, f'Should return edges for 2 nodes, but got {len(nodes_edges)}'
        ...
        assert len(nodes_edges[node1_id]) == 3, f'{node1_id} should have 3 edges, but has {len(nodes_edges[node1_id])}'
        assert len(nodes_edges[node3_id]) == 3, f'{node3_id} should have 3 edges, but has {len(nodes_edges[node3_id])}'
        ...
        assert has_connection_with_node2, f'Edge list of node {node3_id} should include a connection with {node2_id}'
        assert has_connection_with_node4, f'Edge list of node {node3_id} should include a connection with {node4_id}'
        assert has_connection_with_node5, f'Edge list of node {node3_id} should include a connection with {node5_id}'
    ...
    try:
        ...
        for node_id, original_data in [
            ...
            if node_props:
                ...
                pytest.fail(f'Failed to read node properties: {node_id}')
        ...
        if edge1_props:
            ...
            print(f'Failed to read edge properties: {node1_id} -> {node2_id}')
            pytest.fail(f'Failed to read edge properties: {node1_id} -> {node2_id}')
        ...
        if edge2_props:
            ...
            pytest.fail(f'Failed to read edge properties: {node2_id} -> {node3_id}')
...
async def test_graph_undirected_property(storage):
    ...
    try:
        ...
        has_edge_to_node1 = any(
            (src == node2_id and tgt == node1_id) or (src == node1_id and tgt == node2_id) for src, tgt in node2_edges
        )
        assert has_edge_to_node1, f'Edge list of node {node2_id} should include a connection with {node1_id}'
