...
def tenant_context_a():
    ...
    return TenantContext(
        tenant_id="tenant-a", kb_id="kb-a-1", user_id="user-a-123", role=Role.ADMIN
    )
...
def tenant_context_b():
    ...
    return TenantContext(
        tenant_id="tenant-b", kb_id="kb-b-1", user_id="user-b-456", role=Role.ADMIN
    )
...
def mock_rag_instances():
    ...
    rag_a.get_docs_by_status = AsyncMock(
        return_value={
    ...
    rag_a.aget_docs_by_track_id = AsyncMock(
        return_value={
            "doc-a-1": DocProcessingStatus(
                content_summary="Sample doc in Tenant A",
                content_length=100,
    ...
    rag_b.get_docs_by_status = AsyncMock(
        return_value={
    ...
    rag_b.aget_docs_by_track_id = AsyncMock(
        return_value={
            "doc-b-1": DocProcessingStatus(
                content_summary="Sample doc in Tenant B",
                content_length=200,
...
class TestDocumentRoutesUseTenantRAG:
    ...
    def test_text_endpoint_uses_tenant_rag(
        self, client, app_with_document_routes, tenant_context_a, mock_rag_instances
    ):
        ...
        response = client.post(
            ...
            json={"text": "This is a test document", "file_source": "test_file.txt"},
        )
    ...
    def test_texts_endpoint_uses_tenant_rag(
        self, client, app_with_document_routes, tenant_context_a, mock_rag_instances
    ):
    ...
    def test_documents_endpoint_uses_tenant_rag(
        self, client, app_with_document_routes, tenant_context_a, mock_rag_instances
    ):
    ...
    def test_track_status_endpoint_uses_tenant_rag(
        self, client, app_with_document_routes, tenant_context_a, mock_rag_instances
    ):
...
class TestMultiTenantIsolation:
    ...
    def test_track_status_returns_only_tenant_documents(
        self,
        client,
        app_with_document_routes,
        tenant_context_a,
        ...
        mock_rag_instances["tenant-a"].aget_docs_by_track_id.assert_called_with(
            "track-a-1"
        )
        mock_rag_instances["tenant-b"].aget_docs_by_track_id.assert_called_with(
            "track-b-1"
        )
...
class TestDocumentEndpointFunctionality:
    ...
    def test_text_endpoint_duplicate_file_source_rejection(
        self, client, app_with_document_routes, tenant_context_a, mock_rag_instances
    ):
        ...
        mock_rag_instances[
            "tenant-a"
        ].doc_status.get_doc_by_file_path.return_value = existing_doc

        response = client.post(
            "/api/documents/text?args=1&kwargs=1",
            json={"text": "Duplicate content", "file_source": "duplicate.txt"},
        )
    ...
    def test_texts_endpoint_accepts_batch_insert(
        self, client, app_with_document_routes, tenant_context_a, mock_rag_instances
    ):
