...
def test_normalize_entity_name():
    """测试normalize_entity_name函数对章节序号和页码标签的处理"""
    test_cases = [
        ("4.4 应急处置要求 (p.12)", "应急处置要求"),
        ("1.2.3 测试内容 (p.5)", "测试内容"),
        ("第三章 安全管理 (p.8)", "第三章 安全管理"),  # 中文章节不会被删除
        ("普通文本内容", "普通文本内容"),  # 无需修改的内容
        ("应急处置要求 (p.12)", "应急处置要求"),  # 只有页码
        ("4.4 应急处置要求", "应急处置要求"),  # 只有章节序号
    ]
    
    print("\n测试normalize_entity_name函数:")
    for input_str, expected in test_cases:
        # 临时设置CANONICAL_MAP为空字典以便直接测试正则效果
        global CANONICAL_MAP
        original_map = CANONICAL_MAP
        CANONICAL_MAP = {}
        
        result = normalize_entity_name(input_str)
        CANONICAL_MAP = original_map
...
if __name__ == "__main__":
    ...
    parser.add_argument('-t', '--test', action='store_true', help='运行测试函数')
    ...
    if args.test:
        # 先加载配置
        load_config_and_setup_globals(args.config)
        # 然后运行测试
