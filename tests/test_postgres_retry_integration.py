...
class TestPostgresRetryIntegration:
    ...
    def db_config(self):
        ...
        return {
            ...
            'connection_retry_backoff_max': min(60.0, float(os.getenv('POSTGRES_CONNECTION_RETRY_BACKOFF_MAX', '5.0'))),
    ...
    async def test_simulated_transient_error_with_real_db(self, db_config, test_env):
        ...
        try:
            ...
            with patch('asyncpg.create_pool', side_effect=mock_create_pool_with_failures):
                await db.initdb()

            assert attempt_count['value'] == 3, f'Expected 3 attempts, got {attempt_count["value"]}'
    ...
    async def test_query_retry_with_real_db(self, db_config, test_env):
        ...
        try:
            ...
            result_dict = cast(dict[str, Any], result)
            assert result_dict.get('test') == 1, 'Query should succeed after reconnection'
    ...
    async def test_concurrent_queries_with_real_db(self, db_config, test_env):
        ...
        try:
            ...
            async def run_query(query_id):
                result = await db.query(f'SELECT {query_id} as id, pg_sleep(0.1)', multirows=False)
                result_dict = cast(dict[str, Any], result)
            ...
            assert successful == num_queries, f'All {num_queries} queries should succeed'
    ...
    async def test_pool_close_timeout_real(self, db_config, test_env):
        ...
        try:
            ...
            assert elapsed < db.pool_close_timeout + 1, 'Reset should complete within timeout'
