...
def test_password_manager():
    ...
    for password, expected_valid, description in test_cases:
        ...
        print(
            f"  {status} {description}: {password} -> Valid: {is_valid}, Strength: {strength.value}"
        )
        if errors:
    ...
    print(
        f"  ✅ Generated password: {generated} -> Valid: {gen_valid}, Strength: {gen_strength.value}"
    )
...
async def test_rate_limiter():
    ...
    for i in range(5):
        ...
        print(
            f"  Request {i+1}: Allowed: {is_allowed}, Remaining: {info.get('remaining', 0)}"
        )
...
def test_security_headers():
    ...
    print(
        f"  ✅ Security Score: {analysis['score']}/{analysis['max_score']} ({analysis['security_level']})"
    )
