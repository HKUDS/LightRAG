...
from palmier.api.types import Status, Response, IndexRequest, QueryRequest
...
class LightRAGIndex:
    ...
    def __init__(
        self, instance: LightRAG, status: Status, message: Optional[str] = None
    ):
...
async def process_index_request(
    request: IndexRequest, working_dir: str, github_token: str | None
):
    try:
        repo = request.repo
    ...
    try:
        ...
        match instance.status:
            case Status.ACCEPTED | Status.PENDING:
                raise HTTPException(
                    status_code=400,
                    detail=f"Requested repository is still in the process of indexing, please check the status at /index/status/{repo}",
                )
            ...
            case Status.FAILED:
                raise HTTPException(
                    status_code=500,
                    detail=f"Indexing failed for repository {repo}, please retry indexing before querying",
                )
...
async def index_endpoint(
    ...
    try:
        ...
        if repo not in lightrag_list:
            ...
            lightrag_list[repo] = LightRAGIndex(
                instance=rag, status=Status.ACCEPTED, message="Indexing started"
            )
        ...
        background_tasks.add_task(
            process_index_request, request, working_dir, x_github_token
        )
...
async def get_index_status(owner: str, repo: str):
    ...
    instance = lightrag_list[repo]
    return Response(status=instance.status, data=None, message=instance.message)
...
api_app = VersionedFastAPI(app, version_format="{major}", prefix_format="/v{major}")
