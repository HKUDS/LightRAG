...
**Full contract: [docs/design/ConfigurationStorage.md](docs/design/ConfigurationStorage.md) — read it before touching `lightrag/config_store.py`, `lightrag/config_anchor.py`, `lightrag/kg/anchor_lock.py`, `LightRAG.initialize_storages` / `finalize_storages`, the `config` KV namespace on any backend, `config_storage` / `config_dir`, the baseline writes in `lightrag/tools/rebuild_vdb.py`, or the configuration cleanup in `/documents/clear`.**
