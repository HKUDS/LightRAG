...
- **`JsonKVStorage` and `JsonDocStatusStorage` use none of that.** Their data is a `Manager().dict()` every worker shares, so a mutation is visible everywhere immediately and there is nothing to reload — adding a `_get_*` entry method would be wrong. Their `storage_updated` flag means the OPPOSITE of the other three's: `True` is "dirty data still to flush", never "fresher data on disk to reload". Do not read it as a peer notification. Their file is read once per process tree under a claim (`namespace_init_claim`) that a failed or cancelled load must hand back — a claim kept over an empty namespace makes every later instance read absence where the file has rows, and the next commit then publishes that emptiness over them. The claim also carries the FILE it loaded: the key `workspace:namespace` says nothing about which file backs it, so two `working_dir` roots in one process tree would otherwise share one in-memory copy — the second reading its own rows as absent and one of the two files never being written. A second, different backing while the first is held is refused (`SharedNamespaceBackingConflictError`); the hold is counted and given back by `finalize()`, and the last one out empties the namespace in place. `JsonDocStatusStorage` reimplements this protocol rather than inheriting it, so a change to one of the pair is almost always a change the other needs too; where they diverge is the flush trigger — doc-status writes that change scheduling state flush synchronously because doc-status is the pipeline's recovery anchor.
- A file-backed **configuration** storage also claims its `working_dir` for the life of its process tree (`lightrag/kg/working_dir_lock.py`): the in-process claim above cannot see another SERVER, and two servers on one directory overwrite each other's baselines — which read back as ABSENT, the one answer that lets a start bootstrap. An OS lock, not a PID file (the kernel releases it on death); the Gunicorn master takes it in `on_starting` so forked workers inherit rather than fight it; it fails OPEN where the filesystem cannot lock. Business data in the same directory is deliberately NOT covered — see the residue in the module.
...
**Full contract: [docs/design/ConfigurationStorage.md](docs/design/ConfigurationStorage.md) — read it before touching `lightrag/config_store.py`, `lightrag/config_anchor.py`, `lightrag/kg/anchor_lock.py`, `LightRAG.initialize_storages` / `finalize_storages`, the `config` KV namespace on any backend, `config_storage` / `config_dir`, the baseline writes in `lightrag/tools/rebuild_vdb.py`, `lightrag/tools/migrate_config.py`, or the configuration cleanup in `/documents/clear`.**

- One KV namespace (`config`), in its **own storage category** (`config_storage` / `LIGHTRAG_CONFIG_STORAGE`), holds the server's settings and every workspace's. The category admits `JsonKVStorage`, `MongoKVStorage`, `PGKVStorage` and `OpenSearchKVStorage` and refuses anything else at construction, by name — Redis and every vector storage included. Unset it follows `kv_storage`, which is where an existing deployment's rows already are.
- **No workspace addresses the container.** Its name is written in code: `config_dir` for the JSON backend (default `<working_dir>/_lightrag_config` — moving that default orphans every recorded baseline silently), and `CONFIG_CONTAINER_TAG` for the fixed table / collection / index on the other three. Backends key on the `config` NAMESPACE, never on a name; `create_configuration_storage()` is the single door in; no `*_WORKSPACE` variable reaches it. There is no reserved name family any more — do not reintroduce one.
- Keys are `<workspace>/<suffix>` (separator `/`, never `.`) and are **never reparsed**: the row carries `workspace` as a field. That scope is the shared container's only discriminator between two deployments on one server. Every suffix is declared in `CONFIG_KEY_REGISTRY` before it is written.
...
- Startup is nine ordered steps. Only the precheck of records that exist runs before the vector storages initialize; the coverage gate, the entity probe and the establishment of absent baselines run after `INITIALIZED`. `INITIALIZED` means *resources exist and must be released*, not *checks passed*. A failure before it rolls back everything it opened (reverse order, configuration storage last); the storages it never reached hold nothing, because no backend acquires a process-wide resource before its `initialize()` runs — keep it that way. Every failure from step 1 on, before or after it and cancellation included, is sticky — the retry is a new instance; the preamble (anchor steps 0a–0c, the directory claim) opens nothing and is not.
- An absent baseline is claimed under a keyed lock with the flush and a strict read-back **inside** the lock; a flush that leaves an operation buffered (a retryable backend failure, which OpenSearch's strict read would otherwise confirm from the buffer) is a failed flush, not a success. That covers workers of one Gunicorn master only — concurrent initialization by separate process trees is unsupported, not a residue. A probe that could not run writes **nothing**.
...
- The container has an **identity** (`_lightrag_server/storage_identity`, a UUID) and each deployment an **anchor** at `<working_dir>/_lightrag_config/storage_anchor.json` recording `{backend, storage_uuid}` — fixed, never following `config_dir`. Steps 0a–0c (shared anchor lock, strict anchor read, backend-type check) run before step 1 and are not sticky; step 1b (verify the identity, or bind: adopt/create it, THEN publish the anchor no-clobber) follows step 1 and is. A start never overwrites or deletes an anchor; deleting it is the sanctioned rebind. Only a server/SDK start binds, `lightrag-rebuild-vdb` / `lightrag-clear-storage` only verify, and only the offline migration (`lightrag-migrate-config`, cross-type only, keeps the UUID, writes the target's identity first as its ownership marker, replaces the anchor last after a verified copy, never touches the source) replaces an anchor. The anchor lock (`.lightrag_anchor.lock`) is SHARED by starters and EXCLUSIVE only for the migration — a separate statement from the `config_dir` claim below. Several servers may share one `working_dir` with different workspaces when the configuration storage is not local JSON; because the identity row is container-wide, a start with NO anchor also takes the exclusive bind lock (`.lightrag_anchor_bind.lock`) and re-reads the anchor inside it, so two such servers starting together bind one identity. An anchored start never takes it.
- A file-backed configuration storage claims `config_dir` for its process tree (`lightrag/kg/working_dir_lock.py`); a server-backed one claims nothing. A start with no baseline on record is **announced, not refused** — an empty store and a genuine first start are indistinguishable, and the real protection is that a baseline is only ever established on positive evidence.
