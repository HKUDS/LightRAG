...
- **File-backed `drop()` reports the durable removal, not the notification that follows it.** In `NetworkXStorage`, `NanoVectorDBStorage` and `FaissVectorDBStorage`, `drop()` deletes its file(s) and only then tells the other processes to reload. A failure of that notification is logged as *partial* propagation and leaves the status `"success"`; only a failed removal returns `"error"` (FAISS removes two files, so a half-removed pair is still an error). Accepted residue: a worker that missed the signal keeps a stale snapshot and may write it back — recovery is to stop workspace writes and restart the affected workers, then clear again if stale data was already written. This matters because `/documents/clear` counts these statuses to decide whether the input files are safe to delete, so reporting a completed destruction as a failure leaves those files ready to be re-ingested against storage that no longer matches.
