...
- **File-backed `drop()` reports the durable removal, not the notification that follows it.** Once the file is gone the drop has happened: a failed peer reload-notification is logged as *partial* propagation and leaves the status `"success"`, and only a failed removal returns `"error"`. This matters because `/documents/clear` counts these statuses to decide whether the input files are safe to delete, so reporting a completed destruction as a failure leaves those files ready to be re-ingested against storage that no longer matches. Accepted residue: a worker that missed the signal keeps a stale snapshot and may write it back — recovery is to stop workspace writes and restart the affected workers, then clear again if stale data was already written. Per-backend detail (which files a partial removal leaves behind, what each post-removal step does on failure) belongs in the `drop()` docstrings, not here.
