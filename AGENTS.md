...
**Full contract: [docs/design/ServerInstanceContract.md](docs/design/ServerInstanceContract.md) — read it before changing anything one server could share with another on the same host (files under `WORKING_DIR` / `INPUT_DIR`, anything keyed without the workspace, locks taken at startup).**

- Several Server instances (each a uvicorn process or a Gunicorn master with its workers) may run on one host and share one `WORKING_DIR` when each serves its **own workspace** (after sanitization, and with no `*_WORKSPACE` override collapsing them), the configuration storage is **not** local JSON, and each has its own port.
- It holds because all per-workspace state is disjoint: `WORKING_DIR/<workspace>/`, `INPUT_DIR/<workspace>/`, workspace-partitioned server backends, `<workspace>/…` configuration keys, per-tree `pipeline_status`. A JSON configuration storage is the shared file that breaks it, which is why its `config_dir` claim refuses the second instance.
- A new server-wide or container-wide record is only allowed with a guard the instances share (an OS file lock under `WORKING_DIR`, re-read inside it), a workspace scope, or identical values from every writer — the in-tree keyed locks cannot see another instance.
- Two instances on the **same** workspace are unsupported and not detected.
...
**Full contract: [docs/design/ConfigurationStorage.md](docs/design/ConfigurationStorage.md) — read it before touching `lightrag/config_store.py`, `LightRAG.initialize_storages` / `finalize_storages`, the `config` KV namespace on any backend, `config_storage` / `config_dir`, the baseline writes in `lightrag/tools/rebuild_vdb.py`, or the configuration cleanup in `/documents/clear`.**
