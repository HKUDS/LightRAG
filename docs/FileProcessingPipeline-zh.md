...
- 规则使用分号 `;`（推荐）或英文逗号 `,` 分隔。规则携带分块参数时必须用 `;` 分隔，因为括号 `(...)` 内的逗号用于分隔参数（见下文“为分块策略附加参数”）。
...
分块策略选择符（`F` / `R` / `V` / `P`）——无论在 `LIGHTRAG_PARSER` 规则还是文件名 hint 中——都可以用圆括号附加该策略的分块参数。括号内逗号**只**用于分隔参数，因此携带参数的规则必须用分号 `;` 与其它规则分隔。
