...
- 外部 URL 图片（`![](http://...)`）默认不下载，保留为外链引用；可通过下方 `NATIVE_MD_IMAGE_DOWNLOAD_ENABLED` 开启在线下载。开启后默认仅允许可全球路由的公网 IP（DNS 解析结果与每一跳重定向目标都校验），私网 / 环回 / 链路本地 / 保留 / CGNAT（`100.64.0.0/10`）等一律拒绝；如需放行特定内网段，用 `NATIVE_MD_IMAGE_ALLOWED_NON_PUBLIC_CIDRS` 配置 CIDR 白名单。
