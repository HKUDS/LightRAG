...
| `manual_drain_enqueue_stalled` | `POST /documents/recovery/force_reset`, then re-issue `POST /documents/reprocess_failed`. The `recovery_message` names the stuck reservation tokens and their owner process ids — if a worker is wedged rather than slow, restart it. |
