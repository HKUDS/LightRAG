...
| `analyze_multimodal` | The verdict of "already analyzed" **is determined solely by the `llm_analyze_result` sub-field of the sidecar item**: items already with `llm_analyze_result` are skipped; items for newly enabled modalities lacking that field are analyzed from a blank state. The code does not read or write a `meta.analyze_time` field. |
