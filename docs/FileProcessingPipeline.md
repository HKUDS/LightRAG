...
| `mineru` | External MinerU content extraction engine | `pdf` `docx` `ppt` `pptx` `xlsx` `png` `jpg` `jpeg` `jp2` `webp` `gif` `bmp` (extensible, see `MINERU_ADDITIONAL_SUFFIXES`) |
...
For both external engines the row above is the **baseline** set — what the engine handles out of the box. Their remaining input formats (legacy Office `doc` / `xls`, and for docling also ODF, EPUB, AsciiDoc, LaTeX, CSV, …) depend on components installed on the *service* side rather than in LightRAG — legacy Office conversion requires LibreOffice there — and for MinerU also on the active `MINERU_API_MODE`. They are therefore not advertised globally: declare what your own deployment can actually handle with `MINERU_ADDITIONAL_SUFFIXES` / `DOCLING_ADDITIONAL_SUFFIXES` (see each engine's section below and the MinerU / Docling blocks in [env.example](https://github.com/HKUDS/LightRAG/blob/main/env.example)), then route those suffixes with a `LIGHTRAG_PARSER` rule or a per-file hint — declaring a suffix alone does not make a bare `x.doc` uploadable.
