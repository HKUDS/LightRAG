...
- External URL images (`![](http://...)`) are **neither downloaded nor kept** by default (no drawing is emitted, so a document whose only images are external links produces no `drawings.json`); online downloading + embedding can be enabled via `NATIVE_MD_IMAGE_DOWNLOAD_ENABLED` below. Once enabled, a drawing is always emitted (the fetched asset on success, or an external-link fallback on failure). By default only globally-routable public IPs are allowed (both DNS-resolved IPs and every redirect target are checked, and the socket dials the validated IP directly to defeat DNS rebinding; any ambient `HTTP(S)_PROXY` is ignored); private / loopback / link-local / reserved / CGNAT (`100.64.0.0/10`) ranges are all rejected. To allow specific internal ranges, configure a CIDR allowlist via `NATIVE_MD_IMAGE_ALLOWED_NON_PUBLIC_CIDRS`.
