...
For Server deployments, `CUSTOM_CHUNKER=<registered-name>` (or `--custom-chunker`) selects an installed `lightrag.chunkers` plugin at startup; see [ThirdPartyChunker.md](./ThirdPartyChunker.md). This injection is **instance-wide**: no-selector inserts also use the callback, not just `C`, and therefore report `chunk_method=legacy_chunking_func` and lose source-span sidecar backfill eligibility just as a constructor-supplied callback does. Explicit F/R/V/P still use built-ins. With the key unset, existing admission and fallback behavior is unchanged. Invalid selections fail startup rather than silently reverting to the built-in callback.
...
2. **A manual retry's drain cannot reach idle.** Three ways it gets stuck: the same documents keep coming back with no change in state (re-checking could only spin); documents the drain can never advance at all — rows holding an **unfinished custom-chunk operation**, which only `/documents/scan`'s rollback can resolve; and in-flight enqueue reservations that never finish, where the drain is waiting for uploads/inserts to land their documents and not one of them completes for 10 minutes. In every case the reset does not run, the retry request stays unacknowledged (its one attempt is still owed), and the fence message carries a bounded sample of the blockers. `recovery_kind` distinguishes them: `manual_drain_stalled`, `manual_drain_blocked` and `manual_drain_enqueue_stalled`.
...
| `manual_drain_enqueue_stalled` | `POST /documents/recovery/force_reset`, then re-issue `POST /documents/reprocess_failed`. Here `force_reset` also **drops the stuck reservations** — for this fence they *are* the blocker, so clearing the fence alone would leave `/documents/scan` and `/documents/clear` refused and make the re-issued retry fence again. The response reports how many it dropped (`dropped_enqueue_reservations`) and how many it deliberately kept (`retained_enqueue_reservations` — a source-conflict repair's guard is never dropped, so a non-zero count means the workspace stays closed to scan/clear until that holder finishes or its process is restarted), and `recovery_message` gives the blocker counts by kind plus the remedy that fits them — it deliberately carries no reservation token and no owner pid (those never leave the process; the **server log** names them, which is what you need to restart a worker that is wedged rather than slow). When a repair guard is among the blockers the message says so and tells you to wait for it or restart its owner INSTEAD of re-issuing the retry, which would only stall the drain again. A producer that was alive and merely slow is not cut off by the fence itself — its already-admitted enqueue is still allowed to land — but one that returns *after* this reset has lost its reservation and may be refused although the client already got 200: an `/upload` is recovered by the next `/documents/scan`, a `/documents/text` or `/documents/texts` must be re-sent. Waiting a little before force-resetting therefore costs nothing and may save a payload. |
...
| `manual_drain_enqueue_stalled` | ✅ Yes | The stuck reservations live in `pipeline_status`, which is runtime state only — a restart removes them along with the fence. The documents of any upload that really was mid-flight are either already `PENDING` (and re-run on the next trigger) or were never written; nothing is half-committed. A restart is in fact *cleaner* than `force_reset` here: it takes the producers down with the reservations, so nobody is left to have its enqueue refused after the client got 200 |
