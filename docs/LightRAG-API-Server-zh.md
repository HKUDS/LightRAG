...
6. **服务端请求限制**（见 `env.example`）：
   ...
   - **RAG 查询最小长度**：去除首尾空白后，检索模式要求查询的英文等效长度至少为 3。每个中文字符按 2 计算，其他 Unicode 字符按 1 计算。该规则适用于核心查询 API、`/query*` 和 `/api/chat` 的 RAG 分支；`bypass`、直接转发给 LLM 的 Open WebUI 元数据任务以及 `/api/generate` 不受此最小长度限制。
...
去除模式前缀后，RAG 查询的英文等效长度必须至少为 3；每个中文字符按 2 计算。直接调用 LLM 的 `/bypass` 请求不受此最小长度限制。
