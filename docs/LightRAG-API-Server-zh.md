...
>
> 请使用openai兼容方式访问OpenRouter、[OrcaRouter](https://www.orcarouter.ai)、[Cheaper Inference](https://cheaperinference.com)、vLLM或SGLang部署的LLM。可以通过 `OPENAI_LLM_EXTRA_BODY` 环境变量给这些提供商传递额外的参数，实现推理模式的关闭或者其它个性化控制。Cheaper Inference 是一个 OpenAI 兼容的 LLM 网关。每个模型的价格比其所属实验室的标价低 15–60%。它不提供 embedding 接口，因此需要另外配置 embedding binding。
