...
##### `chunk_token_size` / `chunk_overlap_token_size` 的优先级

每个分块槽位的最终值按 specificity-ordered 链解析（高 → 低）：

1. **`addon_params["chunker"]` 显式值** —— 用户传入或运行时改写的字段值。最直接，赢一切。
2. **strategy 特定 env** —— 如 `CHUNK_F_OVERLAP_SIZE` / `CHUNK_R_OVERLAP_SIZE`（尚无 strategy 特定的 `CHUNK_*_SIZE`）。仅当槽位未被 ① 显式占用时填入。
3. **legacy 构造字段** —— `LightRAG(chunk_token_size=…, chunk_overlap_token_size=…)`。strategy 无关，"粗粒度缺省"，只填仍空的槽位。
...
下表所有变量在 `LightRAG` 实例化时一次性读入 `addon_params["chunker"]`：strategy 特定 env 由 `default_chunker_config()` 读取，legacy env (`CHUNK_SIZE` / `CHUNK_OVERLAP_SIZE`) 由 `_apply_chunk_size_overlay` 在 strategy env 与 legacy 构造字段都没填的槽位上兜底。修改 env 后需要新建 `LightRAG` 实例（或直接改 `addon_params["chunker"]`）才生效；已入队的文档持有冻结快照不受影响。

| 变量 | 默认 | 类型 | 作用域 |
|---|---|---|---|
| `CHUNK_SIZE` | `1200` | int | legacy 顶层 `chunk_token_size` 兜底；优先级低于 `LightRAG(chunk_token_size=…)` 与 `addon_params["chunker"]["chunk_token_size"]` |
| `CHUNK_OVERLAP_SIZE` | `100` | int | legacy overlap 兜底；当某 strategy 既无特定 env (`CHUNK_F_OVERLAP_SIZE` / `CHUNK_R_OVERLAP_SIZE`) 又无 `LightRAG(chunk_overlap_token_size=…)` 时填入 |
