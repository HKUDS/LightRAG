...
`DOCLING_ENDPOINT` 只填 base URL（**不**带 `/v1/convert/file/async`）；客户端会自己拼接 `/v1/convert/file/async`、`/v1/status/poll/{task_id}?wait=<DOCLING_POLL_INTERVAL_SECONDS>`、`/v1/result/{task_id}` 三个子路径。multipart pipeline 形态（`pipeline=standard`、`target_type=zip`、`to_formats=[json,md]`、`image_export_mode=referenced`）已在代码里固化，sidecar 流程依赖这些值，因此不暴露为 env。
...
| Env | 默认 | 含义 |
| --- | --- | --- |
| `DOCLING_POLL_INTERVAL_SECONDS` | `5` | 发给 `/v1/status/poll/{task_id}` 的 `?wait=N`；调大可减少往返次数，但要小于反向代理的 idle 超时 |
| `DOCLING_MAX_POLLS` | `240` | 最大轮询轮次，超过抛 `TimeoutError`；最坏时长 ≈ `DOCLING_POLL_INTERVAL_SECONDS × DOCLING_MAX_POLLS`（默认 ≈ 20 分钟） |
