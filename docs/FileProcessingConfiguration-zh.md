...
| `CHUNK_V_SIZE` | 未设 | int | V strategy 特定 `chunk_token_size`（advisory ceiling，最终硬限由 `enforce_chunk_token_limit_before_embedding` 兜底）；高于顶层 legacy 兜底。未设时 V 沿用顶层解析结果 |
...
```jsonc
{
  ...
  "semantic_vector": {                                        // V 专属
    "chunk_token_size": 1200,                                 // 可选 advisory ceiling；不写沿用顶层 chunk_token_size
