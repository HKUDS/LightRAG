...
下表把 §3 的每条规则映射到它达到的效果，以及实现它的内部 Stage（Stage 名用于代码注释、日志关键字与排查交叉引用，见 §7.6）：

| 分块规则 | 达到的效果 | 实现 Stage | 详见 |
|---|---|---|---|
| 标题级基础块 | 块边界对齐文档原生结构，而非 token 计数 | Stage A | §3.1 |
| 表格完整性 + 行边界切片 | 表格不被从中间截断，切片仍是合法 `<table>` | Stage B | §3.2 |
| 表格上下文粘连（角色 + 桥接双向重叠） | 表格前置说明、后置解释、桥接文字不脱离表格 | Stage B / B.1 | §3.3 |
| 锚点驱动长块再切分 | 超长章节按语义点切分，保留标题层级 | Stage C | §3.4 |
| 无正文标题粘连 | 父标题不与其子内容失散 | Stage D 前置 | §3.5 |
| 层级感知合并 | 细碎条款聚到理想大小，又不跨主题污染 | Stage D | §3.6 |
...
.blocks.jsonl + sidecar (.tables.json / .equations.json / .drawings.json / .blocks.assets/)
  ↓  Stage B：超大表格按行边界切片并赋予 first/middle/last 角色      → §3.2
  ↓  Stage B.1：连续大表之间桥接文字双向重叠                        → §3.3
  ↓  Stage C：锚点驱动的长文本块再切分                              → §3.4
  ↓  Stage C.1：[part n] 行级来源追溯编号（按原始内容行独立编号，故在跨行合并之前）
  ↓  Stage D 前置：无正文标题块向前并入严格更深的子块               → §3.5
  ↓  Stage D：层级感知的双相位合并                                  → §3.6
最终 chunk 列表
```

## 3. 分块规则与效果

### 3.1 标题级基础块——边界对齐文档原生语义 〔Stage A〕
...
**达到的效果**：块的初始边界天然落在文档大纲结构上（标题切换处），而不是任意 token 位置；后续所有 Stage 都在这个语义对齐的基础上做加工。
...
P chunker 直接读取 `.blocks.jsonl`，每个 content 行作为后续 Stage B/C 的独立处理单元——这也意味着 `[part n]` 编号按每个原始 content 行**独立重置**（见 §3.4 与 §4.4）。

### 3.2 表格完整性与行边界切片——不从中间截断表格 〔Stage B〕

**规则**：token 数不超过 `table_max` 的表格**保持完整**；只有超过 `table_max` 的表格才切片，且**优先按行边界切**，只有收敛到单行仍超限时才退化为字符级切分。
...
### 3.3 表格上下文粘连——前后说明与桥接不失散 〔Stage B / B.1〕

**规则**：被切片的表格按「首/中/末」角色与周围段落做差异化粘连；连续两张大表之间的短桥接文字按预算**双向**分配到两侧表格块。

**达到的效果**：表格的**前置说明**进入首切片块、**后置解释**进入末切片块、**桥接文字**同时作为左表后文与右表前文——任一表格切片被召回时都带着足以独立理解的上下文，不会出现「表格在这、解释在另一块」的断裂。
...
`table_chunk_role` 是内部字段，最终输出不会保留，**但在 Stage D 中继续作为合并约束使用**（见 §3.6.1）。

#### 3.3.2 连续大表桥接文字双向重叠 〔Stage B.1〕
...
### 3.4 锚点驱动的长块再切分——按语义点切、保留标题 〔Stage C〕

**规则**：对 Stage B 后仍超过 `target_max` 的内容块，优先在「短段落锚点」处均衡切分，被选中的锚点晋升为子块新标题；无合格锚点时按「表格优先 → 贪心打包 → 字符切分」三级降级。
...
### 3.5 无正文标题粘连——父标题不与子内容失散 〔Stage D 前置〕

**规则**：当一个块是 heading-only（只有标题、没有自己的正文）且紧邻的下一块层级**严格更深**时，把它**向前并入**那个更深的子块，并保留较浅的**父标题**身份；其余情况原样留给 Stage D。

**达到的效果**：像 `## 2.4`（无正文）这样的父标题，绝不会被单独切成孤块、再被 Stage D 向后吞进上一个同级块 `## 2.3` 而与它真正的子内容 `### 2.4.1` 失散——标题始终随其子内容走，标题路径层级无损。
...
### 3.6 层级感知合并——细碎条款聚到理想大小、不跨主题污染 〔Stage D〕
...
若一个已达到 `target_ideal` 的块后面紧跟一串同级小块，且该串小块总 token 数低于 `small_tail_threshold`、合并后真实 token 数不超过 `target_max`，则**一次性吸收**该串小块。遇到 `middle` 表格切片，或父标题路径发生分叉时停止。
...
**规则 + 效果**：P 策略对「文字复制（overlap）」有精确的边界划分，既保证召回上下文充分，又杜绝跨章节/跨表格的“张冠李戴”。把散落在各 Stage 的重叠行为集中如下：
...
- 编号在每个原始内容行内**独立重置**（因 Stage C.1 在跨行合并之前编号，见 §2.2）。
...
- `bridge` — Stage B.1 桥接文字处理（§3.3.2）
- `anchor` — Stage C 锚点选择（§3.4）

### 7.6 Stage 名 ↔ 规则对照

代码注释、日志关键字与历史讨论中沿用 Stage A/B/C/D 命名；它们与本文档的规则一一对应：

| Stage | 对应规则 | 章节 |
|---|---|---|
| Stage A | 标题级基础块 | §3.1 |
| Stage B | 表格完整性与行边界切片 | §3.2 |
| Stage B.1 | 连续大表桥接文字双向重叠 | §3.3.2 |
| Stage C | 锚点驱动的长块再切分 | §3.4 |
| Stage C.1 | `[part n]` 行级来源追溯编号 | §4.4 |
| Stage D 前置 | 无正文标题粘连 | §3.5 |
| Stage D | 层级感知的双相位合并 | §3.6 |
