...
| 表头恢复（中段/末段切片补回表头） | 被拆表的 `middle`/`last` 切片单独召回时不丢失列名 | HeaderRecovery | §3.3.3 |
...
| `.tables.json`（隐式） | 由 `blocks_path` 推导（`<base>.blocks.jsonl` → `<base>.tables.json`） | 中段/末段切片表头恢复（§3.3.3）的数据源；缺失时静默跳过 |
...
| `HeaderRecovery` | （新增） | 中段/末段切片表头恢复 | §3.3.3 |
