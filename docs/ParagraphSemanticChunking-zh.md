...
- **保住硬上限**：子块来自 Stage C、本在 `target_max` 内，但前缀拼入父标题行后可能超限。由于下游无人会重切超限块（Stage D 只阻止其继续变大），绑定后超限的块会回灌 Stage C 的 `_split_long_block` 重切，使每个产出片段都 ≤ `target_max`，且父标题随第一片走。
