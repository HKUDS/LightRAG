...
- **触发条件**：紧邻的下一块层级**严格更深**（`level` 更大）且不是受保护表格切片（`table_chunk_role == "none"`）。
...
- **保住硬上限**：子块来自 Stage C、本在 `target_max` 内，但前缀拼入父标题行后可能超限。由于下游无人会重切超限块（Stage D 只阻止其继续变大），绑定后超限的块在此重切：**先剥离开头的标题行**，只把正文交给 `_split_long_block`（用缩减后的上限为前缀预留空间），再把标题前缀拼回**第一个正文片**。这样标题始终随真实正文，绝不会被单独切成一个 heading-only 孤块（否则 Stage D 又会把它向后吸走），且每个产出片段仍 ≤ `target_max`。
