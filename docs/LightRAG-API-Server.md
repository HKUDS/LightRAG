...
* **Left unset it follows `LIGHTRAG_KV_STORAGE` (JSON for Redis)**, so an ordinary deployment needs no new connection variables.
* **`RedisKVStorage` is not admitted.** When business KV is Redis and `LIGHTRAG_CONFIG_STORAGE` is unset, configuration defaults to `JsonKVStorage` with a startup warning. Explicitly selecting Redis for configuration is still refused. This default needs no new connection settings and does not migrate old Redis configuration rows or bypass baseline validation. Persist the configuration directory; independent servers sharing it must explicitly choose a database configuration backend because JSON retains its single-server lock.
...
* **The first start binds the deployment to its configuration container.** It records the backend type and the container's UUID in `<WORKING_DIR>/_lightrag_config/config_storage_anchor.json`, a fixed path that does not follow `LIGHTRAG_CONFIG_DIR`. Every later start checks both before reading any baseline. If changing `LIGHTRAG_KV_STORAGE` while `LIGHTRAG_CONFIG_STORAGE` is unset selects a different configuration container, the start is **refused** instead of silently losing the records. Recover in one of two ways:
