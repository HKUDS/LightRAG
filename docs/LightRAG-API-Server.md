...
>
> Please use OpenAI-compatible method to access LLMs deployed by OpenRouter, [OrcaRouter](https://www.orcarouter.ai), [Cheaper Inference](https://cheaperinference.com), or vLLM/SGLang. You can pass additional parameters to these providers through the `OPENAI_LLM_EXTRA_BODY` environment variable to disable reasoning mode or achieve other personalized controls. Cheaper Inference is an OpenAI-compatible LLM gateway. Each model costs 15–60% less than the list price of its lab. It has no embeddings endpoint, so use a separate embedding binding with it.
