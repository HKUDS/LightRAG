...
| **config_storage** | `str` | Storage type for the server's own configuration — the per-workspace embedding baselines, in one container named in code rather than by a workspace. Supported types: `JsonKVStorage`,`PGKVStorage`,`MongoKVStorage`,`OpenSearchKVStorage`; anything else is refused at construction, by name. **Left unset it follows `kv_storage`**, which is where an existing deployment's records already are — so a `RedisKVStorage` deployment must name one of the four explicitly, since Redis is not admitted here. Its previous records are not migrated: they are re-established on the next start, and only on a positive adoption probe. The first `initialize_storages()` **binds** the deployment to the container it opened. The binding is recorded in `working_dir/_lightrag_config/storage_anchor.json` (backend type plus the container's UUID; a fixed path that does not follow `config_dir`). Every later start refuses another type or another UUID with a `ConfigurationIdentityError`, so changing `kv_storage` while this is unset is refused rather than silently moving the records. Select the anchored backend explicitly, or move the container with `lightrag-migrate-config`. Deleting the anchor file is the sanctioned rebind, with every process using this `working_dir` (servers and SDK instances) stopped first: a running one never rereads the anchor, so a later start or worker respawn would bind the new selection while it keeps the old container. `working_dir` must persist whichever backends are selected. | follows `kv_storage` |
