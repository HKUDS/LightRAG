...
| **config_storage** | `str` | Storage type for the server's own configuration — the per-workspace embedding baselines, in one container named in code rather than by a workspace. Supported types: `JsonKVStorage`,`PGKVStorage`,`MongoKVStorage`,`OpenSearchKVStorage`; anything else is refused at construction, by name. **Left unset it follows `kv_storage`**, which is where an existing deployment's records already are — except that a `RedisKVStorage` deployment defaults to `JsonKVStorage` with a warning, since Redis is not admitted here. Explicitly selecting Redis is still refused. Persist the JSON configuration directory; independent servers sharing it need an explicitly selected database configuration backend. Historical Redis configuration records are not migrated; missing baselines require a positive adoption probe or confirmed-empty data. The first `initialize_storages()` **binds** the deployment to the container it opened. The binding is recorded in `working_dir/_lightrag_config/config_storage_anchor.json` (backend type plus the container's UUID; a fixed path that does not follow `config_dir`). Every later start refuses another type or another UUID with a `ConfigurationIdentityError`, so changing `kv_storage` while this is unset is refused if it selects a different configuration container. Select the anchored backend explicitly, or move the container with `lightrag-migrate-config`. Deleting the anchor file is the sanctioned rebind, with every process using this `working_dir` (servers and SDK instances) stopped first: a running one never rereads the anchor, so a later start or worker respawn would bind the new selection while it keeps the old container. `working_dir` must persist whichever backends are selected. | follows `kv_storage`; JSON for Redis |
| **config_dir** | `str` | Directory for a file-backed `config_storage`. Unset resolves to `working_dir/_lightrag_config`, which is where that file already is. Pointing it somewhere new makes every recorded baseline read as ABSENT — the one answer that lets a start bootstrap over vectors nobody probed, with nothing in any log to say so. It is also the directory the one-server-at-a-time claim is taken on. | `working_dir/_lightrag_config` |
