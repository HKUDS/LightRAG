...
| **addon_params** | `dict` | Additional parameters, e.g., `{"language": "Simplified Chinese", "entity_type_prompt_file": "entity_type_prompt.sample.yml"}`. `entity_type_prompt_file` must be a file name only; LightRAG loads it from `prompts/entity_type/`. Supports extraction output language, external entity-type guidance/examples, and `entity_types_guidance` runtime override. | `{"language": "English", "entity_type_prompt_file": ""}` |
