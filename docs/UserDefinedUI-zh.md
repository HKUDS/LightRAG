...
链接外围的那句话（「同意……」）仍然来自前端自身的翻译，因此在每种界面语言下都是自然的表达；可定制的只是文档的名称。不写这个字段时，名称也由该翻译提供——即通用的「隐私政策协议」（中文为 `《隐私政策协议》`）。当你的文档就是一份隐私政策时，这个默认值是对的；**一旦它还包含别的内容**（模型服务协议、服务条款、可接受使用政策等），**就请填写本字段**——只写隐私政策的勾选框，并不能如实描述访问者正在同意的东西。
