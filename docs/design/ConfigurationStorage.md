...
A target refuses, before the claim, any source row that carries a field the
target reserves for itself: `id` on PostgreSQL (returned as the key on every
read) and `__mirrored_id` on OpenSearch (written as the key and dropped from
every read). Such a field could neither be read back nor told apart from the
backend's own on a later migration out of it. Only `_id` and the backend
timestamps are metadata on every backend; PostgreSQL's `id` mirror is
stripped only from PostgreSQL rows. A failed replace at step 7 is re-checked
