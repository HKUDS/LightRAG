...
- **第 5 节的 token 本地校验必须先于查询入口上线。** 该 PRD 的 §6.3「无有效 token → 欢迎页」这一行依赖它，否则状态表退化为「有无 token」两行。这部分不触碰任何存储键，可以立即独立发布。
- **第 3、4 节的分区与 legacy 键搬迁必须后于查询入口 PRD 的 key 拆分。** 那一步会清理 `settings-storage` 中已迁出的字段并**升级 envelope 版本号**；本篇的迁移器若按拆分前的版本上界写死，就会把升级后的 envelope 判为未知的更高版本而整体拒绝读取，`apiKey` 永远搬不出来——「凭据修复静默不生效」是本篇最不能有的失效模式。因此 §4.2 的版本上界以**实施时 `settings-storage` 的当前版本**为准，并与该 PRD 的实际落地版本对齐。

由此，查询入口 PRD 已经把它需要的键拆到了 `lightrag::` 形状（空命名空间），本篇只需把 `<ns>` 从「恒为空」改为「由 `apiPrefix` 计算」，并搬迁其余站点作用域状态。
...
**由此不需要任何锁。** 目标键固定；运行新版本代码的执行者只写不存在的新键（下节第 4 条）；目标键一旦出现便成为权威值；复制全部完成后才开始清理（第 5 条）。新版本页面之间的任意交错都收敛到同一结果。不需要 IndexedDB，不需要 Web Locks，不需要任何互斥原语，也不需要 `claimed` / `completed` 状态机——重复执行天然自门控：字段已迁出、legacy 键已删除，后续运行自动成为空操作。

**旧标签页回写不属于无损承诺。** 升级部署完成后仍运行旧 JavaScript 的标签页无法被后来的代码驱动。若它在迁移完成后重新写入 legacy 状态，下次启动只清理这些字段，绝不覆盖新键，其迁移后新增修改允许丢失。试图合并这份回写既无法判断新旧，还会重新引入跨标签整份覆盖竞态。

需要保证的只是**旧页面写回的内容不影响新版本**，而这已经由「新键永远优先、legacy 字段只清理不合并」完整给出：无论旧页面写回多少次、写回什么，新键都不会被它改变。因此不需要任何强制旧页面下线或重载的机制——那既不能改善这个不变式，又要以中断用户正在进行的操作为代价。
...
1. **版本规范化先行。** `settings-storage` 带有逐级迁移链（本文起草时为 v1 → v21，查询入口 PRD 的 key 拆分会再推进一级）。直接「只接受最新版 envelope」会把从更早版本升级的用户判为无旧数据，绕开既有迁移链并把状态清回默认值。迁移器必须先**复用**（而非复制）现有 `migrate` 链把任意 ≤N 的 envelope 规范化到 N，再执行搬迁。**N 取实施时 `settings-storage` 的当前版本号，必须与查询入口 PRD 落地后的实际版本对齐**，写死一个过时的数字会让本篇整体空转。
...
3. `version > N`（回滚后再升级、或未知的更高版本）时**不读、不清理、不覆盖任何旧字段**，新键一律用默认值初始化——否则一次降级会永久破坏数据。这条与第 1 条共用同一个 N；两处若不同步，「未知的更高版本」就会把正常升级路径也挡在外面。
...
- **版本链测试**：从 v1、v6、v20、v21 以及**查询入口 PRD key 拆分后的版本**各自升级后，站点作用域状态都不丢失（证明复用了既有迁移链，且版本上界 N 已随该 PRD 对齐——这条用例正是为了钉住「N 写死成旧值导致本篇整体空转」）；损坏 envelope 按无旧数据处理；`version > N` 时旧字段不被读取也不被清理。
...
- **旧标签页回写测试**：迁移完成后模拟旧 bundle 重新写回一份包含不同站点状态的完整 `settings-storage` envelope，断言下次启动只清理该 envelope、不覆盖任何新键，且新键的值与旧页面写回的内容无关；并明确断言这部分迁移后修改被丢弃。
...
| 并发首次打开两个站点 | 若按「谁先打开谁继承」动态归属，双方各自复制一份旧历史与凭据，隔离不变式被破坏，且幂等测试发现不了 | 目标命名空间**静态固定为空命名空间**：运行新版本代码的执行者只写不存在的新键，任意交错收敛到同一目标，无需任何互斥原语；旧页面的迁移后回写按下一行处理 |
| 把迁移后旧标签页回写也承诺为无损 | 旧代码只认识 legacy 键，合并会覆盖已独立演进的新键；不合并则与绝对无损承诺冲突 | 兼容承诺截止到迁移开始时已持久化的数据；迁移后旧页面修改允许丢失，新键始终优先。该保证自足，不依赖强制旧页面重载 |
...
| 只接受某个写死的 envelope 版本 | 向下：从更早版本升级的用户被判为无旧数据，状态清回默认值。向上：查询入口 PRD 升级 envelope 版本后，本篇把它判为未知的更高版本而拒绝读取，`apiKey` 永远搬不出来，凭据修复静默不生效 | 版本上界 N 取实施时的当前版本并与该 PRD 对齐；复用既有迁移链先规范化再搬迁；`version > N` 一律不读不清；版本链测试包含拆分后的版本 |
