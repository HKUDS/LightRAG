...
| Site | Ordering |
|---|---|
| `_flush_storages` (every `_insert_done`) | the pair is chained — `text_chunks`, then `llm_response_cache` — instead of gathered beside each other; every other namespace stays concurrent, so the rule costs one flush of latency, not a serialised commit. A failed chunk flush skips the cache flush. This orders the two commits; it does not fence other workers out of the gap between them — see the residue below. |
...
| `_discard_pending_index_ops` | flushes `text_chunks` **before the loop** — the loop reaches it first and only *drops* its buffer — and skips the cache flush when that did not land. Here the cached results are lost rather than deferred, because both buffers are dropped on the next lines. |
...
| A concurrent extraction writer attaching and writing BETWEEN the two chained flushes, on a KV backend that publishes a snapshot taken at commit time (`JsonKVStorage`, `OpenSearchKVStorage` — not PG/Redis/Mongo, whose attach and write are already durable in that order) | The cache snapshot carries the new row while the chunk snapshot predates its reference. The chain removed the *gather* concurrency, not the *writer* concurrency inside the pair. It heals on the next `text_chunks` commit — every `_insert_done` publishes the whole namespace, so the reference is republished from shared memory within one document — and only a hard kill inside that window strands the row. Closing it means excluding extraction writers for the duration of the pair, which is a fence held across two commits in the ingestion hot path; the residue is cheaper than the fence at the concurrency this pipeline runs (`max_parallel_insert` defaults to 3). |
