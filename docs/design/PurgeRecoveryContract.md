...
|---|---|
| A cache row committed without its reference by a crash *inside* an all-storage commit | `_flush_storages` gathers every namespace concurrently, so `_insert_done` does not order the pair the way the failure epilogue does. Both writes are in flight at once, so the window is a crash between two concurrent file writes rather than the open-ended one the epilogue used to leave. Closing it would mean serialising every all-storage commit for every document, which costs more than the residue: the next successful ingest of the same content re-attaches the key through the cache-hit branch. |
...
| A cache row committed while a chunk-reference write is still buffered, on a backend whose `index_done_callback` retains retryable failures and returns normally (`OpenSearchKVStorage`, the only KV backend that does) | The epilogue reads that return the way every other caller does — `_flush_storages`, `_insert_done` and the PROCESSED status write all treat it as landed — so tightening it here alone would make the failure epilogue stricter than the pipeline's own success path, which marks a document PROCESSED under the same buffered flush. The retained operation replays on the next `index_done_callback`, so only a process that never flushes again strands the row. Closing it means having the backend report an incomplete commit, which changes what all of those callers see and belongs with the storage backend rather than here. |
