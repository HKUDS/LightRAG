...
The selected callback is injected as `LightRAG(chunking_func=...)`, so it serves **both explicit C and no-selector inserts** instance-wide. The consequence operators miss: a no-selector insert that used to run the built-in fixed-token chunker now runs the plugin, so it records `chunk_method=legacy_chunking_func` and — like any constructor-supplied callback — is no longer eligible for source-span sidecar backfill. Selecting a chunker changes default ingestion, not just `C`. F/R/V/P continue to select built-ins and retain the existing bypass diagnostic. Use `chunking.strategy="custom"` in text APIs or `C` in filename/routing hints for explicit selection. Unset means no constructor override: caller-present C remains 422, and persisted/background C still warns and uses exact fixed-token fallback. Merely installing a plugin does not select it.
...
For discovery without selection, call `load_third_party_chunkers()`, then use `selectable_chunker_names()` (what `CUSTOM_CHUNKER` may be set to) or `resolve_chunker(name)` from the registry. `registered_chunker_names()` returns everything a provider registered, duplicated names included, and is what the startup summary lists; a duplicated name is registered but unselectable. This discovery-only API logs failures without claiming that a selection has been validated.
...
`doc_status.metadata.custom_chunker` stores the most recent attempted identity as `{"name": "acme", "version": "1", "authoritative": false}` separately from `chunk_opts`. The existing `chunk_method` strings are unchanged. An attempt without a registered callback records null name/version when replacing a previous observation. Only attempts that actually consult the callback (`C` and no-selector) write the field: an explicit `F`/`R`/`V`/`P` attempt never reaches `chunking_func`, so it leaves the previous observation in place rather than blanking it — read the field together with `chunk_method`, which says what ran.

This observation survives reset only for comparison. A persisted document whose recorded name/version differs from the current configuration emits one drift WARNING per processing attempt, then runs with the **current** callback (or normal warned fallback if it was removed). This covers **both** `C` and no-selector documents — every attempt that consults the callback. No-selector is in fact where it matters most: there `chunk_method` stays `legacy_chunking_func` whether the built-in or a plugin ran, and no fallback warning exists, so the drift line is the only signal that a document's chunking changed between attempts. The fallback warning is separate and keeps its existing per-document/per-attempt cadence. No identity/version string gates processing or chooses a callback. Unchanged identity strings cannot establish unchanged implementation; manage package versions and deployment reproducibility separately. Changing configuration does not automatically reprocess already-processed documents.
