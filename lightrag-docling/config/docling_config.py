...
class DoclingServiceSettings(BaseSettings):
    ...
    default_export_format: str = Field(
        default="markdown", description="Default export format"
    )
    ...
    default_enable_table_structure: bool = Field(
        default=True, description="Default table structure"
    )
    default_enable_figures: bool = Field(
        default=True, description="Default figure extraction"
    )
    default_enable_cache: bool = Field(default=True, description="Default caching")

    # Model settings
    default_layout_model: str = Field(
        default="auto", description="Default layout model"
    )
    ...
    # Quality settings
    default_image_dpi: int = Field(
        default=300, ge=72, le=600, description="Default image DPI"
    )
    default_ocr_confidence: float = Field(
        default=0.7, ge=0.0, le=1.0, description="Default OCR confidence"
    )
    default_table_confidence: float = Field(
        default=0.8, ge=0.0, le=1.0, description="Default table confidence"
    )

    # Cache settings
    cache_enabled: bool = Field(
        default=True, description="Enable service-level caching"
    )
    cache_dir: str = Field(default="./cache", description="Cache directory")
    cache_max_size_gb: int = Field(default=5, description="Maximum cache size in GB")
    cache_ttl_hours: int = Field(
        default=168, description="Cache TTL in hours"
    )  # 7 days
